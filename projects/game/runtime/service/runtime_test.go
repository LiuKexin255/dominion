...
func newTestService(t *testing.T, runtimeID string, verifier token.Verifier) *RuntimeService {
	t.Helper()
	signer := token.NewHMACSigner("test-secret", 1*time.Hour)
	cfg := &config.RuntimeConfig{
		RuntimeID: runtimeID,
	}
	svc := NewRuntimeService(sessionmanager.NewManager(runtimeID), NewControlExecutor(), cfg, signer, verifier)
	return svc
}

func TestRuntimeService_ConnectSession(t *testing.T) {
	tests := []struct {
		name          string
		...
		{
			name:          "valid token with matching runtime and session",
		...
		{
			name:          "wrong runtime ID rejected",
			pathSessionID: "session-1",
			claims: &token.Claims{
				SessionID:      "session-1",
				OwnerRuntimeID: "rt-other",
				OwnerEpoch:     1,
				ExpiresAt:      time.Now().Add(5 * time.Minute).Unix(),
			},
			runtimeID: "rt-0",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newTestService(t, tt.runtimeID, &stubVerifier{claims: tt.claims, err: tt.verifyErr})
			...
			if claims.OwnerRuntimeID != tt.runtimeID {
				t.Fatalf("claims.OwnerRuntimeID = %q, want %q", claims.OwnerRuntimeID, tt.runtimeID)
			}
		})
	}
}
...
func TestCreateGameRuntime(t *testing.T) {
	...
	svc := NewRuntimeService(sessionmanager.NewManager("rt-0"), NewControlExecutor(), cfg, signer, signer)
	...
	if rt.OwnerRuntimeID != "rt-0" {
		t.Fatalf("rt.OwnerRuntimeID = %q, want %q", rt.OwnerRuntimeID, "rt-0")
	}
	if rt.OwnerEpoch != 1 {
	...
	if claims.OwnerRuntimeID != "rt-0" {
		t.Fatalf("claims.OwnerRuntimeID = %q, want %q", claims.OwnerRuntimeID, "rt-0")
	}
...
func TestRuntimeService_ProcessHello_Agent(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	rt := svc.sessions.GetOrCreateRuntime("session-1")
	claims := &token.Claims{SessionID: "session-1", OwnerRuntimeID: "rt-0"}
...
func TestRuntimeService_ProcessHello_AgentAlreadyConnected(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	rt := svc.sessions.GetOrCreateRuntime("session-1")
	claims := &token.Claims{SessionID: "session-1", OwnerRuntimeID: "rt-0"}
...
func TestRuntimeService_ProcessHello_Web(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	rt := svc.sessions.GetOrCreateRuntime("session-1")
	claims := &token.Claims{SessionID: "session-1", OwnerRuntimeID: "rt-0"}
...
func TestRuntimeService_ProcessHello_WebNoCache(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	rt := svc.sessions.GetOrCreateRuntime("session-1")
	claims := &token.Claims{SessionID: "session-1", OwnerRuntimeID: "rt-0"}
...
func TestRuntimeService_HandleAgentMessage_MediaInit(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
...
func TestRuntimeService_HandleAgentMessage_MediaSegment(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleAgentMessage_ControlAck(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleAgentMessage_ControlResult(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleAgentMessage_ControlResultFlashSnapshot(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleWebMessage_ControlRequest(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleWebMessage_ControlRequestInvalid(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleWebMessage_Ping(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_HandleWebMessage_UnknownPayload(t *testing.T) {
	svc := newTestService(t, "rt-0", &stubVerifier{})
	svc.sessions.GetOrCreateRuntime("session-1")
...
func TestRuntimeService_GetSnapshot(t *testing.T) {
	now := time.Now()

	tests := []struct {
		name       string
		sessionID  string
		setupCache func(svc *RuntimeService)
		wantErr    bool
		wantData   string
		wantCached bool
	}{
		...
		},
		{
			...
			setupCache: func(_ *RuntimeService) {
			},
			wantErr: true,
		},
	}
...
func TestRuntimeService_GetRuntime(t *testing.T) {
	tests := []struct {
		name      string
		sessionID string
		wantErr   bool
...
func newRefreshTestService(t *testing.T, runtimeID string) (*RuntimeService, *token.HMACSigner) {
	t.Helper()
	...
	svc := NewRuntimeService(sessionmanager.NewManager(runtimeID), NewControlExecutor(), cfg, signer, signer)
	return svc, signer
}

func TestRefreshGameRuntime_SameOwner(t *testing.T) {
	svc, signer := newRefreshTestService(t, "rt-0")
	...
	oldToken, err := signer.Issue("session-1", "rt-0", 1, token.TokenAudienceInternal, 3)
	if err != nil {
	...
	if gotRT.OwnerRuntimeID != "rt-0" {
		t.Fatalf("OwnerRuntimeID = %q, want %q", gotRT.OwnerRuntimeID, "rt-0")
	}
	...
	if claims.OwnerRuntimeID != "rt-0" {
		t.Fatalf("new token OwnerRuntimeID = %q, want %q", claims.OwnerRuntimeID, "rt-0")
	}
	if claims.OwnerEpoch != 2 {
...
func TestRefreshGameRuntime_TakeoverRejected(t *testing.T) {
	...
	oldToken, err := signer.Issue("session-1", "rt-1", 5, token.TokenAudienceInternal, 2)
	if err != nil {
		t.Fatalf("issue old token: %v", err)
	}
...
func TestRefreshGameRuntime_RuntimeNotFound(t *testing.T) {
	...
	oldToken, err := signer.Issue("session-1", "rt-0", 1, token.TokenAudienceInternal, 5)
	if err != nil {
		t.Fatalf("issue old token: %v", err)
	}
...
func TestRefreshGameRuntime_InvalidToken(t *testing.T) {
	svc, _ := newRefreshTestService(t, "rt-0")
...
func TestRefreshGameRuntime_WrongSession(t *testing.T) {
	...
	oldToken, err := signer.Issue("session-other", "rt-0", 1, token.TokenAudienceInternal, 1)
	if err != nil {
		t.Fatalf("issue old token: %v", err)
	}
...
func TestRefreshGameRuntime_OldTokenWithLowerEpochAllowed(t *testing.T) {
	...
	oldToken, err := signer.Issue("session-1", "rt-0", 1, token.TokenAudienceInternal, 2)
	if err != nil {
...
func TestRefreshGameRuntime_NewerTokenEpochRejected(t *testing.T) {
	...
	oldToken, err := signer.Issue("session-1", "rt-0", 5, token.TokenAudienceInternal, 2)
	if err != nil {
...
func TestRefreshGameRuntime_GraceWindow(t *testing.T) {
	...
	oldToken, err := pastSigner.Issue("session-1", "rt-0", 1, token.TokenAudienceInternal, 2)
	if err != nil {
