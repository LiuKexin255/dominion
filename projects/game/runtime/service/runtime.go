...
import (
	...
	"dominion/projects/game/runtime/domain/mediacache"
	"dominion/projects/game/runtime/domain/sessionmanager"
)

var (
	errRuntimeMismatch = errors.New("runtime ID mismatch")
...
func (s *RuntimeService) AsyncMessages() <-chan *domain.RoutedMessage {
	return s.asyncCh
}
...
// was cancelled.
func (s *RuntimeService) HandleCompletion(ctx context.Context, comp domain.ControlCompletion) bool {
	logs.Info(ctx, "runtime: handling control completion",
...
func (s *RuntimeService) StartCompletionWorker() bootstrap.WorkerBuilder {
...
// runtime ID matches this instance, and ensures the session ID in the token
// matches the path parameter.
func (s *RuntimeService) ConnectSession(ctx context.Context, pathSessionID, tokenStr string) (*domain.SessionRuntime, *token.Claims, error) {
	...
	if claims.OwnerRuntimeID != s.config.RuntimeID {
		return nil, nil, errRuntimeMismatch
	}
	...
	logs.Info(ctx, "runtime: session connected",
...
func (s *RuntimeService) ProcessHello(rt *domain.SessionRuntime, _ *token.Claims, role domain.ClientRole, connID string) ([]*domain.RoutedMessage, error) {
	switch role {
...
// random access point for a late-joining web client.
func (s *RuntimeService) buildCatchUpMessages(sessionID string) []*domain.RoutedMessage {
...
func (s *RuntimeService) HandleAgentMessage(_ context.Context, sessionID string, msg *domain.Message) ([]*domain.RoutedMessage, error) {
...
func (s *RuntimeService) handleMediaInit(sessionID string, init domain.MediaInitPayload) ([]*domain.RoutedMessage, error) {
	cache := s.getOrCreateMediaCache(sessionID)
...
func (s *RuntimeService) handleMediaSegment(sessionID string, seg domain.MediaSegmentPayload) ([]*domain.RoutedMessage, error) {
	cache := s.getOrCreateMediaCache(sessionID)
...
func (s *RuntimeService) handleControlAck(sessionID string, ack domain.ControlAckPayload) ([]*domain.RoutedMessage, error) {
...
func (s *RuntimeService) handleControlResult(sessionID string, result domain.ControlResultPayload) ([]*domain.RoutedMessage, error) {
...
// HandleWebMessage processes messages received from a web connection.
func (s *RuntimeService) HandleWebMessage(_ context.Context, sessionID string, connID string, msg *domain.Message) ([]*domain.RoutedMessage, error) {
	switch p := msg.Payload.(type) {
...
func (s *RuntimeService) handleControlRequest(sessionID, connID string, req domain.ControlRequestPayload) ([]*domain.RoutedMessage, error) {
...
func (s *RuntimeService) handlePing(sessionID, connID string, ping domain.PingPayload) ([]*domain.RoutedMessage, error) {
	return []*domain.RoutedMessage{
		{
			TargetKind:   domain.RouteTargetConn,
			TargetConnID: connID,
...
func (s *RuntimeService) GetSnapshot(_ context.Context, sessionID string) (*domain.SnapshotRef, error) {
	rt := s.sessions.GetRuntime(sessionID)
	if rt == nil {
...
// DisconnectAgent removes the agent connection for the session.
func (s *RuntimeService) DisconnectAgent(sessionID string) {
	s.sessions.UnregisterAgent(sessionID)
	s.control.HandleAgentDisconnect(sessionID)
}

// DisconnectWeb removes a web viewer connection from the session.
func (s *RuntimeService) DisconnectWeb(sessionID, connID string) {
	s.sessions.RemoveWebConn(sessionID, connID)
}

// TouchSession refreshes the idle TTL for a session.
func (s *RuntimeService) TouchSession(sessionID string) error {
	return s.sessions.TouchSession(sessionID)
}

// GetRuntime returns the current session runtime state.
func (s *RuntimeService) GetRuntime(_ context.Context, sessionID string) (*domain.SessionRuntime, error) {
	rt := s.sessions.GetRuntime(sessionID)
	if rt == nil {
		return nil, domain.ErrSessionNotFound
	}
...
// CreateGameRuntime creates an in-memory game runtime for the session.
func (s *RuntimeService) CreateGameRuntime(_ context.Context, sessionID string, reconnectGeneration int64) (*domain.SessionRuntime, string, error) {
	rt := s.sessions.GetOrCreateRuntime(sessionID)

	if rt.OwnerRuntimeID == s.config.RuntimeID && rt.ReconnectGeneration > 0 {
		if reconnectGeneration > rt.ReconnectGeneration {
			rt.ReconnectGeneration = reconnectGeneration
		}
		rt.OwnerEpoch++
	} else {
		rt.OwnerRuntimeID = s.config.RuntimeID
		rt.OwnerEpoch = 1
		rt.ReconnectGeneration = reconnectGeneration
	}
	...
	tokenStr, err := s.tokenIssuer.Issue(sessionID, s.config.RuntimeID, rt.OwnerEpoch, token.TokenAudienceInternal, rt.ReconnectGeneration)
	if err != nil {
...
// RefreshGameRuntime refreshes the token for an existing runtime.
func (s *RuntimeService) RefreshGameRuntime(_ context.Context, sessionID, oldToken string) (*domain.SessionRuntime, string, error) {
	...
	if claims.OwnerRuntimeID != s.config.RuntimeID {
		return nil, "", fmt.Errorf("token owned by %q, this runtime is %q", claims.OwnerRuntimeID, s.config.RuntimeID)
	}
...
// getOrCreateMediaCache returns the media cache for the given session.
func (s *RuntimeService) getOrCreateMediaCache(sessionID string) domain.MediaCache {
	s.mediaMu.Lock()
