...
func TestManager_GetOrCreateRuntime(t *testing.T) {
	...
	t.Run("creates new runtime", func(t *testing.T) {
		...
		if rt.RuntimeID != "rt-0" {
			t.Fatalf("RuntimeID = %q, want %q", rt.RuntimeID, "rt-0")
		}
		if rt.AgentConn != nil {
	...
	t.Run("returns existing runtime on second call", func(t *testing.T) {
		...
		// then - same pointer, runtimeID unchanged
		if second != first {
			t.Fatal("expected same runtime instance")
		}
		if second.RuntimeID != "rt-0" {
			t.Fatalf("RuntimeID = %q, want original %q", second.RuntimeID, "rt-0")
		}
	})
...
func TestCleanup_RemovesIdleSessions(t *testing.T) {
	m := NewManager("rt-0", WithIdleTTL(1*time.Millisecond))
	m.GetOrCreateRuntime("session-1")
...
func TestCleanup_KeepsActiveSessions(t *testing.T) {
	m := NewManager("rt-0", WithIdleTTL(100*time.Millisecond))
	m.GetOrCreateRuntime("session-1")
