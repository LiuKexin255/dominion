...
import { STREAM_IDLE_TIMEOUT_MS, TOOL_HEARTBEAT_INTERVAL_MS } from "./llm";
...
// ------------------------------------------------------------------
// 043 US3 (T008c — specs/043-llm-stream-stall-recovery/contracts/
// stall-recovery-contract.md §1.2, research.md R7): dispatch heartbeat
// ------------------------------------------------------------------
describe("OperationBridge dispatch heartbeat (043 US3, T008c)", () => {
  let bridge: OperationBridge;
  ...
  // The LangGraph idle watchdog (installed dist/pregel/timeout.js
  // `checkIdle`) fires only when `lastProgress + idleTimeout < now`. The
  // dispatch heartbeat refreshes `lastProgress` unconditionally, so as long
  // as no gap between refreshes reaches STREAM_IDLE_TIMEOUT_MS the watchdog
  // can never raise a false `NodeTimeoutError` mid-tool. The real watchdog
  // lives in LangGraph; this unit test verifies the bridge keeps the refresh
  // cadence tight enough (TOOL_HEARTBEAT_INTERVAL_MS < STREAM_IDLE_TIMEOUT_MS,
  // llm.ts) across a dispatch that resolves after the idle window.
  it("heartbeats at TOOL_HEARTBEAT_INTERVAL_MS cadence across a > idle-window dispatch and clears the interval on resolve", async () => {
    bridge.registerSink(() => {});
    ...
    const part = makeMovePart();
    const promise = bridge.dispatch(part, undefined, heartbeat);

    // Immediate first call: the first setInterval tick is
    // TOOL_HEARTBEAT_INTERVAL_MS away, so the idle timer is refreshed at
    // dispatch start (contract §1.2 — no initial gap).
    expect(heartbeat).toHaveBeenCalledTimes(1);

    // Elapse > STREAM_IDLE_TIMEOUT_MS (e.g. 30s idle + 5s) with the dispatch
    // still pending — the window a bare idleTimeout would have fired in.
    const elapsedMs = STREAM_IDLE_TIMEOUT_MS + 5_000;
    await vi.advanceTimersByTimeAsync(elapsedMs);

    // Cadence: 1 immediate + one tick per TOOL_HEARTBEAT_INTERVAL_MS.
    ...
    bridge.handleResult(makeResult(part.mouseMove!.toolId!, STATUS_SUCCEEDED, "ok"));
    await promise;
    heartbeatTimes.push(Date.now()); // resolution point — trailing gap

    // Gap invariant: consecutive refreshes (start → ticks → resolve) are
    // never farther apart than the interval, which is < the idle window, so
    // the idle watchdog could not have elapsed (no false NodeTimeoutError).
    expect(heartbeatTimes.length).toBeGreaterThanOrEqual(2);
    for (let i = 1; i < heartbeatTimes.length; i += 1) {
      expect(heartbeatTimes[i] - heartbeatTimes[i - 1]).toBeLessThanOrEqual(
        TOOL_HEARTBEAT_INTERVAL_MS,
      );
    }

    // Interval cleared on resolve: no leaked timers.
    expect(vi.getTimerCount()).toBe(0);
  });

  it("without a heartbeat (non-saolei path), a dispatch past the idle window stays pending — documents the degraded behavior", async () => {
    bridge.registerSink(() => {});
    const part = makeMovePart();
    let resolved = false;
    const promise = bridge.dispatch(part).then((r) => {
      resolved = true;
      return r;
    });

    // Nothing refreshes the idle timer: at graph level this is exactly the
    // condition under which LangGraph raises NodeTimeoutError after
    // STREAM_IDLE_TIMEOUT_MS (contract §1.2 — acceptable only where the tool
    // is known short). The bridge itself stays pending; only handleResult or
    // the 20-min timeout can resolve it.
    await vi.advanceTimersByTimeAsync(STREAM_IDLE_TIMEOUT_MS + 5_000);
    expect(resolved).toBe(false);

    bridge.handleResult(makeResult(part.mouseMove!.toolId!, STATUS_SUCCEEDED, "late"));
    const result = await promise;
    expect(result.status).toBe(STATUS_SUCCEEDED);
    expect(result.message).toBe("late");
    expect(vi.getTimerCount()).toBe(0);
  });

  it("clears the heartbeat interval when the dispatch aborts", async () => {
    bridge.registerSink(() => {});
    const controller = new AbortController();
    const heartbeat = vi.fn();
    const part = makeMovePart();
    const promise = bridge.dispatch(part, controller.signal, heartbeat);
  ...
  it("clears the heartbeat interval when the dispatch times out (DISPATCH_TIMEOUT_MS)", async () => {
    bridge.registerSink(() => {});
    const heartbeat = vi.fn();
    const part = makeMovePart();
    const promise = bridge.dispatch(part, undefined, heartbeat);
