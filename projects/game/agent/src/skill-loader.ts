 ...
 * research.md D9 — "append the SKILL.md body to the systemPrompt").
 *
 * ---------------------------------------------------------------------------
 * File-access design (deviation — see note below)
 * ---------------------------------------------------------------------------
 *
 * The preferred mechanisms were assessed and found not viable without build
 * infrastructure work that is out of this feature's scope:
 *
 *   - `.md?raw` bundler import (方案 A): production uses ts_project + swc
 *     (no Vite/webpack pipeline), so `?raw` imports are not processed and
 *     fail at runtime ("Cannot find module"). No `?raw` precedent exists in
 *     the repo.
 *   - Runtime `fs.readFileSync` (方案 B): `artifact_pkg_js`
 *     (`tools/release/defs.bzl`) packages only compiled JS, protos, and npm
 *     deps — it does NOT carry arbitrary `.md` data files into the deployed
 *     image. Packaging the `.md` would require extending that rule.
 *
 * Adopted approach (achieves the SPIRIT of 方案 A — a build-time inlined
 * string with NO runtime fs access): the SKILL.md body is inlined here as a
 * TypeScript template-literal constant. The authored `SKILL.md` at
 * `src/skill/saolei/SKILL.md` remains the source of truth (FR-023
 * deliverable); `skill-loader.test.ts` reads the `.md` via
 * `fs.readFileSync` (available in the vitest runfiles via the vitest_test
 * `data`) and asserts byte-equality with this constant — drift between the
 * two is caught at test time.
 */
...
/**
 * The body of the built-in `saolei` skill, WITHOUT frontmatter.
 *
 * Authority: `projects/game/agent/src/skill/saolei/SKILL.md` (FR-023).
 * Drift guard: `skill-loader.test.ts` reads the `.md` file, strips its
 * frontmatter, and asserts equality with this constant.
 *
 * The constant is generated from the SKILL.md body with backticks escaped
 * (`\\``) so it can live inside a TypeScript template literal; the drift
 * test verifies the round-trip is lossless.
 */
const SAOLEI_SKILL_BODY: string = `# saolei
...
/**
 * Registry: `mcp_name` → built-in skill body (Markdown, WITHOUT frontmatter).
 *
 * Spec 018-saolei-mcp FR-024: a profile whose `mcp_names` includes a
 * registered entry receives the matching built-in skill body appended to its
 * systemPrompt. FR-025 scope guard: this registry is the ONLY skill-injection
 * surface; it MUST NOT alter the user-created `Skill` proto resource
 * (PromptService CRUD at `projects/game/game.proto`).
 *
 * To add a new built-in skill: author `src/skill/{name}/SKILL.md`, inline
 * the body as a constant above, and add the `mcp_name → body` entry here.
 */
const BUILTIN_SKILLS: Readonly<Record<string, string>> = {
	saolei: SAOLEI_SKILL_BODY,
};
