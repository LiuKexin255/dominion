...
// ExtractJPEGFallback returns a minimal valid 1×1 white JPEG image.
// Used when the full fMP4→H.264→JPEG pipeline is unavailable (e.g. in tests
// without real video data).
func ExtractJPEGFallback() []byte {
	// Minimal 1x1 white JPEG (SOI + APP0 + SOF0 + DHT + SOS + EOI)
	return []byte{
		...
		0x7B, 0x94, 0x11, 0x00, 0x00, 0x00, 0x00, 0x2E,
		0xA2, 0x80, 0x00, 0x00, 0x00, 0x02, 0xFF, 0xD9,
	}
}
