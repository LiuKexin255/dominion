...
（/api/v2 team 流语义、历史回填、失败/终止语义、连接状态）、preset
分池 CRUD/持久化/物化与模型目录、游戏闭环（won 链路、desktop 缺席）
与 desktop flow 面；断连 suite `game-disconnect`（部署
...
服务的拓扑变体）承载物化 fail-loud 回滚断言。测试替换面：fake-llm `/v1/responses` 替换真实端点、
