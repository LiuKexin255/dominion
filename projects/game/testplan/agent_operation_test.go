...
// (coordinates only) and "mouse_click" (click_type only, at current
// cursor position). The profiles below declare the split tool names so
// the agent compiles with the post-015 tool surface.
package testplan
...
func TestAgentOperationResultSuccess(t *testing.T) {
	...
	// Simulate a desktop-executed operation result. After the US2 split a
	// click fires at the current cursor position (no coordinates carried).
...
// FAILED status (simulating an out-of-bounds mouse_move coordinate) is
...
// post-015 split tool names (mouse_move + mouse_click) binds
// successfully and the agent processes a text turn without error. This
// is a regression guard for the US2 buildTools wiring: the legacy
// single "mouse" name is no longer recognized, so a profile that still
// declared it would silently register zero tools.
//
// Note: fake-llm's stateless keyword-matched Message templates cannot
// initiate a tool_call (only tool-result responses chain further
// calls), so actual mouse_move/mouse_click dispatch is covered at the
// unit level (mouse-tool.test.ts) rather than here. This case confirms
// the split tool names are accepted end-to-end through the real
// AgentAdapterImpl compile path.
