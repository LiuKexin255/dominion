...
func TestArtifactSpecs_ExposureModePersistence(t *testing.T) {
	tests := []struct {
		name         string
		source       *domain.ArtifactSpec
		mongo        mongoArtifactSpec
		...
		{
			name: "old mongo document defaults to unspecified",
			mongo: mongoArtifactSpec{
				Name:     "svc",
				App:      "app",
				Image:    "image:v1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.source != nil {
				...
				if mongoSpecs[0].ExposureMode != int(tt.wantExposure) {
					t.Fatalf("artifactSpecsToMongo() exposure_mode = %d, want %d", mongoSpecs[0].ExposureMode, tt.wantExposure)
				}
			} else {
				mongoSpecs = []mongoArtifactSpec{tt.mongo}
			}
			...
			if got[0].Exposure != tt.wantExposure {
				t.Fatalf("artifactSpecsFromMongo() exposure = %d, want %d", got[0].Exposure, tt.wantExposure)
			}
			if tt.checkRound && got[0].Exposure != tt.source.Exposure {
				t.Fatalf("round trip exposure = %d, want %d", got[0].Exposure, tt.source.Exposure)
			}
		})
	}
}
