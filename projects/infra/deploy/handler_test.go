...
func Test_exposureModeToProto(t *testing.T) {
	tests := []struct {
		input domain.ExposureMode
		want  ExposureMode
	}{
		{domain.ExposureModeUnspecified, ExposureMode_EXPOSURE_MODE_UNSPECIFIED},
		{domain.ExposureModeAggregate, ExposureMode_EXPOSURE_MODE_AGGREGATE},
		{domain.ExposureModePerInstance, ExposureMode_EXPOSURE_MODE_PER_INSTANCE},
		{domain.ExposureMode(99), ExposureMode_EXPOSURE_MODE_UNSPECIFIED},
	}

	for _, tt := range tests {
		t.Run(fmt.Sprintf("domain_%d", tt.input), func(t *testing.T) {
			got := exposureModeToProto(tt.input)
			if got != tt.want {
				t.Fatalf("exposureModeToProto(%v) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}

func Test_exposureModeFromProto(t *testing.T) {
	tests := []struct {
		input ExposureMode
		want  domain.ExposureMode
	}{
		{ExposureMode_EXPOSURE_MODE_UNSPECIFIED, domain.ExposureModeUnspecified},
		{ExposureMode_EXPOSURE_MODE_AGGREGATE, domain.ExposureModeAggregate},
		{ExposureMode_EXPOSURE_MODE_PER_INSTANCE, domain.ExposureModePerInstance},
		{ExposureMode(99), domain.ExposureModeUnspecified},
	}

	for _, tt := range tests {
		t.Run(fmt.Sprintf("proto_%d", tt.input), func(t *testing.T) {
			got := exposureModeFromProto(tt.input)
			if got != tt.want {
				t.Fatalf("exposureModeFromProto(%v) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}

func Test_toProtoArtifacts_fromProtoArtifacts_exposureRoundTrip(t *testing.T) {
	tests := []struct {
		name         string
		domain       []*domain.ArtifactSpec
		wantExposure domain.ExposureMode
	}{
		{
			name: "aggregate round-trip preserves value",
			domain: []*domain.ArtifactSpec{{
				Name:         "api",
				App:          "gateway",
				Image:        "example.com/gateway:v1",
			...
			wantExposure: domain.ExposureModeAggregate,
		},
		{
			name: "per-instance round-trip preserves value",
			domain: []*domain.ArtifactSpec{{
				Name:         "api",
				App:          "gateway",
				Image:        "example.com/gateway:v1",
			...
			wantExposure: domain.ExposureModePerInstance,
		},
		{
			name: "unspecified round-trip preserves value",
			domain: []*domain.ArtifactSpec{{
				Name:         "api",
				App:          "gateway",
				Image:        "example.com/gateway:v1",
				WorkloadKind: domain.WorkloadKindStateless,
				Exposure:     domain.ExposureModeUnspecified,
			}},
			wantExposure: domain.ExposureModeUnspecified,
		},
		{
			name: "zero-value exposure stays unspecified through round-trip",
			domain: []*domain.ArtifactSpec{{
				Name:  "api",
				App:   "gateway",
				Image: "example.com/gateway:v1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got[0].Exposure != tt.wantExposure {
				t.Fatalf("Exposure = %v, want %v", got[0].Exposure, tt.wantExposure)
			}
		})
	}
}
