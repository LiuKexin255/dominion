...
const (
	testServiceAPath        = "//tools/release/deploy/v2/compiler/testdata/service-a.yaml"
	testServiceBPath        = "//tools/release/deploy/v2/compiler/testdata/service-b.yaml"
	testServiceCPath        = "//tools/release/deploy/v2/compiler/testdata/service-c.yaml"
	testServiceStatefulPath = "//tools/release/deploy/v2/compiler/testdata/service-stateful.yaml"
)
...
func TestCompile_StatefulWorkload(t *testing.T) {
	tests := []struct {
		...
		{
			name: "stateful with http config is rejected",
			deployConfig: &config.DeployConfig{
				Services: []*config.DeployService{{
			...
			wantErr: "http is only supported for stateless workloads, got kind=stateful",
		},
		{
		...
		{
			name: "stateful with multiple http matches is rejected",
			deployConfig: &config.DeployConfig{
				Services: []*config.DeployService{{
					Artifact: config.DeployArtifact{Path: testServiceStatefulPath, Name: "service-stateful"},
					HTTP: config.DeployHTTP{
...
func newCompilerWorkspace(t *testing.T) string {
	...
	copyDir(t, filepath.Join(srcRoot, "testdata"), filepath.Join(root, "tools", "release", "deploy", "v2", "compiler", "testdata"))
