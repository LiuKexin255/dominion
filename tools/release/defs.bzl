...
def _artifact_pkg_js_impl(ctx):
    ...
    for target in ctx.attr.runtime_protos:
        ...
        for src in proto_info.transitive_sources.to_list():
            sp = src.short_path
            # Filter well-known protobuf types. Use substring check because
            # Bzlmod may encode repo names as a prefix on short_path (e.g.,
            # "f+/src/google/protobuf/..." instead of "google/protobuf/...").
            if "google/protobuf/" in sp:
                continue
            # Filter Bazel internal virtual-import paths (generated by proto
            # compilation, not real source files).
            if "_virtual_imports" in sp:
                continue

            # Compute canonical import path from short_path.
            canonical = sp
            if src.owner.workspace_name:
                canonical = sp[len(src.owner.workspace_name) + 1:]

            # Bzlmod may encode canonical repository names into short_path even
            # when workspace_name is not set. Encoded prefixes are short and
            # contain '+'. Strip the first component when it looks like a
            # repo-encoding prefix rather than a real proto path directory.
    ...
    # Phase 5: data_files (non-TS resources like .md, .json)
    args.add("--data-files")
    # Service manifest at the tar service root: Node determines the module
    # format of the deployed .js files from the nearest package.json, so
    # dominion/{app}/{service}/package.json must ship in every service tar.
    inputs.append(ctx.file.package_json)
    args.add(ctx.file.package_json.path)
    args.add("package.json")
    for target in ctx.attr.data_files:
        for src in target.files.to_list():
            short = src.short_path
            dest = short[len(pkg_prefix):] if (pkg_prefix and short.startswith(pkg_prefix)) else short
            inputs.append(src)
...
_artifact_pkg_js = rule(
    ...
    attrs = {
        ...
        "data_files": attr.label_list(
            allow_files = True,
            default = [],
            doc = "Additional non-TS data files to include in the tar, placed relative to the package directory (same path logic as ts_project files). Must not carry package.json (destination conflict with the mandatory manifest copy).",
        ),
        "service": attr.string(mandatory = True),
    },
)

def artifact_pkg_js(name, package_json = "package.json", **kwargs):
    """Creates a tar layer for a JS service.

    See ``_artifact_pkg_js`` for the packaging layout. ``package_json``
    defaults to the calling BUILD package's ``package.json`` and is shipped
    at ``dominion/{app}/{service}/package.json`` in the tar: Node determines
    ...
    Args:
        ...
        package_json: Service manifest label; defaults to "package.json" in
            the calling BUILD package.
        **kwargs: Remaining ``_artifact_pkg_js`` attributes (app, service,
            entrypoint, ts_project, runtime_protos, runtime_deps, npm_deps,
            data_files, visibility).
    """
    _artifact_pkg_js(
        name = name,
