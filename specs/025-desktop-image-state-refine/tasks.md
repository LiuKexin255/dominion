...
- [X] T010 [US2] Verify the gRPC server-side max message size for the proxy/agent hops (`research.md` D1 verification item): the gateway client side is already 8 MiB (`gateway/cmd/main.go:48-51`); confirm the proxy/agent server `MaxRecvMsgSize` tolerates the largest expected screenshot (default gRPC server max is 4 MiB) and raise it if a screenshot can exceed that
