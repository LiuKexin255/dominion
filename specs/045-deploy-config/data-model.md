...
- VR-CS-2: 列表内重复选择 → schema `uniqueItems` 拒绝（与 FR-004 配置块名唯一性对称；避免编译出重复 `{block,key}` 的 `ConfigEntry`，与控制面 VR-CE-2 冲突）
- VR-CS-3: deploy.yaml 对配置 MUST NOT 覆盖或修改配置块中的数据（FR-008）—— schema 层 config 仅为字符串列表，无 value/type 字段

**State Transitions**: 无。选择是静态的。

### 4. Config Entry（proto / domain 层）

通过 protobuf 传递到 deploy 控制面的配置数据，携带内联数据以便控制面创建 ConfigMap。

| Field | Type | Proto Field # | Description |
|-------|------|---------------|-------------|
| block | `string` | 1 | 配置块名 |
| key | `string` | 2 | 数据条目名 |
| type | `string` | 3 | 格式类型（`json`/`yaml`），保留以备审计；运行时不使用（一律 YAML 解析） |
| value | `string` | 4 | 原始数据文本（ConfigMap data 的内容） |

**Parent**: `ArtifactSpec.config_entries` (proto field **12**；见下"Proto 变更")

**Validation Rules**（domain `ArtifactSpec.Validate()`）:
- VR-CE-1: block/key/type/value 均非空
- VR-CE-2: 同一 ArtifactSpec 内 `{block,key}` 组合不重复（防 ConfigMap key 与投影路径冲突）
...
|----------|-------------|
| ConfigMap `{workload}-config` | 控制面创建；data key 为 `{block}-{key}`，value 为原始数据文本 |
| Volume `dominion-config` | ProjectedVolume，对每个 ConfigEntry 投影 `ConfigMapProjection{LocalObjectReference: {workload}-config, Items: [{Key: "{block}-{key}", Path: "{block}/{key}"}]}` |
| VolumeMount `/mnt/dominion/config` | 只读挂载 |
| EnvVar `DOMINION_CONFIG_DIR` | 值 `/mnt/dominion/config`，平台强制注入（保留变量） |

**Generated By**: `projects/infra/deploy/runtime/k8s/builder.go` → `BuildDeployment` / `BuildStatefulSet`；ConfigMap 由 `executor.go` 新增 apply 路径创建。

**Conditions**:
- 仅当 artifact 有至少一个 config entry 时创建 ConfigMap / volume / mount / env
- 无 config entries 时不创建任何资源（与 secret 行为一致，见 002 R6）
...
ArtifactSpec (proto / domain)
  └── ConfigEntries: []ConfigEntry            ← {block, key, type, value} 内联数据
                  │
...
K8s Deployment/StatefulSet
  ├── ConfigMap: {workload}-config            ← executor 创建（data: {block}-{key} → value）
  ├── Volume: dominion-config (projected)     ← KeyToPath 还原 {block}/{key} 目录
...
message ArtifactSpec {
  ...
  repeated ConfigEntry config_entries = 12;   // NEW
}
...
message ConfigEntry {
  string block = 1;  // 配置块名
  string key = 2;    // 数据条目名
  string type = 3;   // 格式类型 "json"|"yaml"（审计用，运行时不使用）
  string value = 4;  // 原始数据文本
}
```
...
func (e *ConfigEntry) Validate() error { /* 各字段非空 */ }
...
type DeploymentWorkload struct {
    ...
    ConfigEntries  []*domain.ConfigEntry // NEW
}
```
...
| ConfigEntry {block,key} 重复 | — | — | — | domain ArtifactSpec.Validate()（防御纵深，防非 CLI 客户端） |
