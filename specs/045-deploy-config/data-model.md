...
- VR-CE-2: 同一 ArtifactSpec 内 `{block,key}` 组合不重复。per-block 映射下，重复的 `{block,key}` 即同一 ConfigMap 的同一 data key，结构上必然冲突（map 写覆盖）；domain 校验作为防御纵深保留（防非 CLI 客户端绕过 schema `uniqueItems`，见 [yaml-schema.md §2](contracts/yaml-schema.md)）
...
**Generated By**: `projects/infra/deploy/runtime/k8s/builder.go` → `BuildDeployment` / `BuildStatefulSet`（投影）与 `BuildConfigMaps`（per-block ConfigMap，按 block 首次出现顺序）；ConfigMap 由 `executor.go` 的 per-block apply 路径创建。

**Conditions**:
- 仅当 artifact 有至少一个 config entry 时创建 ConfigMap(s) / volume / mount / env
...
| ConfigEntry {block,key} 重复 | — | — | — | domain ArtifactSpec.Validate()（防御纵深，防非 CLI 客户端；per-block 下即同 ConfigMap 同 data key 冲突） |
