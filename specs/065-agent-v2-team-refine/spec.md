...
- **memory List 排序归属服务端、机制通用、实现以 Go 习惯为准**（2026-09-16 三次用户裁定）：`ListMemories` 增 `order_by`（AIP-132 `{field} [desc]` 通用语法 + 字段白名单映射 `memory_id`/`update_time` + 指定 tie-breaker `memory_id` 收尾 + 通用游标 + 单路径仓储——不为单个排序需求定制方法/字面量/游标），缺省 `memory_id` 升序零破坏；快照装载以 `page_size=10 + order_by` 单页取最近 10 条——客户端不得全量拉取后自排序；第三次裁定进一步约束实现形态：全部校验（语法/白名单/重复/游标键匹配）收敛 domain 层先校验后产出，仓储只做已验证输入直译，类型与签名遵守 `style/golang.md`（结构体容器 `[]*T`、查找返回 `*T`、游标为单结构体 wire 形态无 `*string` 字段）；对外语义（合法 `order_by` 集合、token 形态、排序结果）与 JS 消费面零变化。契约见 `specs/065-agent-v2-team-refine/contracts/memory-snapshot-recency.md` §1。
