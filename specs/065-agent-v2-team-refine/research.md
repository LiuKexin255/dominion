...
**Date**: 2026-09-16
**方法**: 源码级调研（本地物化依赖 0.1.1-rc.2 + 本仓库插件/宿主/服务源码），无外部新依赖引入；排序通用化部分补充外部规范与社区实践调研（AIP-132/158/193、seek method、MongoDB keyset 实践、Firestore/Elasticsearch/Relay 先例）与 Go 语言习惯调研（Google Go Style Guide/Decisions、Go Code Review Comments、Effective Go——第三次裁定后实现形态以 Go 习惯为锚，见 R0，引用均为完整 URL）。所有行号锚点以当前 `main` 工作树为准。
...
- **Go 语言习惯调研（2026-09-16 第三次用户裁定："这一版代码很多不是 Golang 习惯……是否在 designer 调研时过度迁移了其他语言（SQL/JS 系参考）的风格"——本轮实现形态以 Go 官方习惯 + `style/golang.md` 为锚重铸）**：
  ...
  - **仓库内先例（只读参考）**：session 服务 `projects/game/session/domain/pagination.go` + `projects/game/session/runtime/mongo/repository.go`——`cursorJSON` 字段即普通 `string`（无指针）、仓储 `List` 的续页过滤为**字面两子句 `$or`**（`{create_time: {$lt}}`、`{create_time: 等值, session_id: {$lt}}`，无前缀累积循环）、`cursor *domain.ListPageCursor` 为 nil 判首页。其可借鉴点（string 字段、字面两键、nil-首页、limit+1）全部吸收；与本轮裁定的差异：session 是单一排序形态的固定双键游标（typed struct 游标 `ListPageCursor{CreateTime, SessionID}` + cursorJSON 转换），memory 通用机制下游标为排序键形态无关的 entries 数组（单一结构体即 wire 形态，无转换层）——session 先例保持只读参考、不追溯改造（单排序服务无泛化收益）。
...
### D4: 记忆快照近因注入——ListMemories 通用排序机制（Go 习惯与分层重铸；2026-09-16 三次用户裁定）

- **Decision**: `ListMemoriesRequest` 的 `string order_by = 4` 按 **AIP-132 通用语法**解析（`{field} [desc]` 逗号分隔列表、空白不敏感、升序省略后缀），对**字段白名单映射表**（API 字段 → Mongo 字段，domain 包内私有单一事实源；首期 `memory_id`/`update_time`）校验，未知字段/非法后缀/重复字段/`memory_id` 非末位 → INVALID_ARGUMENT。**指定 tie-breaker 收尾**：排序键未以 `memory_id`（指定的唯一键收尾字段）收尾时自动追加 `memory_id asc`（确定全序）；tie-breaker 是一个指定字段而非"扫描 Unique 字段"的通用规则；缺省（空 `order_by`）= 空键列表经收尾规则 = `memory_id` 升序——无缺省分支。**通用游标（单一结构体即 wire 形态）**：`MemoryPageCursor []*MemoryCursorEntry`、`MemoryCursorEntry{Field, Value string}`（json tag 直接标注，无指针字段、无中间 JSON 结构体与转换层），token = base64url(NoPadding) JSON 数组，时间以 RFC3339Nano 字符串承载、按字段类型在使用点转换；不编码方向（token = 位置）；`DecodeMemoryPageToken(token, sort)` 在 domain 一步完成解码 + 键匹配 + 类型校验（跨序重放/坏 token → `ErrInvalidPageToken`）。**分层**：domain 先完整校验后产出（`ParseMemoryOrderBy`）+ 游标解码校验；handler 只做字符串透传与错误映射；仓储收到已验证输入只做直译（`MemorySortTerm{Field, MongoField, Descending}` 自包含键元素，`style/golang.md` 函数节不重复校验）。**seek 条件直白两键形态**：最终键 ∈ {[memory_id], [业务, memory_id]}，单键直接 `{memory_id: {$gt|$lt}}` 并入 filter、两键字面 `$or` 两子句（镜像 session 先例），无 specs/sort/values 平行数组。索引义务约定不变：新增可排序字段 = 白名单一行 + accessor 一行 + 按消费方向建复合索引。JS `listMemories` 可选 `{orderBy, pageSize}`（单页即止/全页累积）；快照装载 `page_size=10 + order_by=update_time desc`；`renderMemorySnapshot` 纯透传；`MemoryEntry = {memory_id, content}`。写路径与快照冻结时机零改动。**wire 兼容性**：`order_by` 合法/非法字符串集合与 token 对外形态（base64url-JSON `[{field,value}]` 字符串对、时间 RFC3339Nano）不变，JS/testplan/front-end 消费面零影响。契约：`specs/065-agent-v2-team-refine/contracts/memory-snapshot-recency.md`。
- **Rationale**: 排序/截取是集合的服务端职责（2026-09-16 第一次裁定），排序机制 MUST 通用（第二次裁定：不为单个排序需求定制方法/字面量匹配/分排序游标），**实现形态 MUST 以 Go 习惯为准**（第三次裁定：SQL/JS 系参考只提供语义——AIP-132 语法、keyset 唯一键收尾、游标位置语义、OR 阶梯数学——不迁移其结构形态进 Go）。外部语义锚点保留第二次裁定调研（R0：AIP-132/158/193、use-the-index-luke seek method、MongoDB keyset 社区实践、Firestore `__name__`/ES tiebreaker/Relay 先例）。Go 形态锚点（R0 本轮调研）：`*string` 字段违反 Pass values（Style Decisions/CodeReviewComments 明示 `*string` 反例）；双结构体 + 转换层、`any` 类型分支编解码违反 Simplicity/Least mechanism；平行数组随行（specs 是 sort 的派生物却作为独立参数到处传递）是 C++ 出参风格的残留（Effective Go Multiple return values："obviates the need to pass a pointer to a return value to simulate a reference parameter"）；仓储内做白名单/匹配校验违反 `style/golang.md` 函数节（已校验入参不重复校验）且把"排序参数 → 查询语句"的直译复杂化；先校验后产出（validate-then-produce）符合"先检查输入正确、再解析"的 Go 习惯与 In-band errors 原则。通用 Unique 扫描收尾被否定的语义理由：保证全序的唯一字段**有一个就够**——tie-breaker 是"指定的收尾字段"这一声明性事实，不是可对任意唯一字段泛化的规则；Firestore `__name__` 亦是指定字段（文档名）而非"任意唯一字段扫描"。seek 条件选择"简化条件构造"方向（而非 repo 闭环）的分层理由：cursor 与排序的匹配校验必须对照**最终键**（含收尾），若 repo 负责收尾则匹配校验必须随之进入仓储或双向重复收尾规则——与"校验收敛 domain"裁定冲突；直白两键形态的完备性由结构导出（白名单恰一个业务字段 + tie-breaker 末位 ⇒ 键长 ≤ 2），并以 domain pin 测试守护该不变量（新增第二个业务字段时测试失败，强制同批泛化仓储 ladder）。
- **Alternatives considered**:
  ...
  - **泛化 n 键前缀累积 ladder**（2026-09-16 第三次裁定否决）：`memorySeekFilter` 以累积循环生成任意键长的 OR 阶梯——最终键由结构限定至多两键（业务 + tie-breaker），泛化机械服务于不可达输入（违反 Least mechanism）；字面两子句（镜像 session `List`）为全部可达形态；未来出现第二个业务字段的多键消费时再有意泛化（pin 测试守护）。
...
| memory 服务 | `projects/game/game.proto` | `ListMemoriesRequest` 增 `string order_by = 4`（AIP-132 通用语法/白名单/指定 tie-breaker 收尾/token 键匹配注释） |
| memory 服务 | `projects/game/memory/domain/sort.go`（新）+ `projects/game/memory/domain/{model,repository,errors}.go` | `MemorySortTerm{Field, MongoField, Descending}`（+包内 kind 与 `CursorValue` 值转换方法）、包内私有白名单 `[]*memorySortFieldSpec{Field, MongoField, kind}`（单一事实源，无 Unique 标志）、`ParseMemoryOrderBy`（先完整校验——语法+白名单+重复+tie-breaker 末位，后产出含指定 `memory_id` 收尾的最终键，返回 `[]*MemorySortTerm`）；`ListMemories` 签名 `(sort []*MemorySortTerm, cursor MemoryPageCursor, pageSize)`；`ErrInvalidPageToken` |
| memory 服务 | `projects/game/memory/domain/pagination.go` | 通用游标（单一结构体即 wire 形态）：`MemoryCursorEntry{Field, Value string}`（json tag 直接标注）+ `type MemoryPageCursor []*MemoryCursorEntry`；`EncodeMemoryPageToken(cursor) string` 全函数（base64url NoPadding JSON、空 cursor → 空 token）；`DecodeMemoryPageToken(token, sort)`（base64/JSON/形态/空串校验 + 与最终键匹配校验 + 经 `CursorValue` 的时间解析校验；全部 wrap `ErrInvalidPageToken`） |
| memory 服务 | `projects/game/memory/handler/handler.go` | `order_by` → `domain.ParseMemoryOrderBy`、token → `domain.DecodeMemoryPageToken(token, sort)`（空 token 跳过）；解析错误与 `ErrInvalidPageToken` → INVALID_ARGUMENT（无排序知识） |
| memory 服务 | `projects/game/memory/runtime/mongo/repository.go` + `model.go` | 单路径 `ListMemories`（已验证输入直译：`memorySortDocument(sort)` MongoField × 方向、seek 条件直白两键形态——单键并入 filter/两键字面 `$or` 两子句、limit+1、`memoryNextPageToken` 经 accessor 构造 entries + 全函数编码）；`memoryDocument.sortValue(field) string` wire 形态 accessor（时间 UTC RFC3339Nano，每白名单字段一个 case）；启动建 `{template, session_id, update_time: -1, memory_id: 1}` 复合索引（既有唯一索引不变） |
