...
- Repository internal prototype (the openai_llm interface testplan, since removed in `specs/048-js-esm-migration/tasks.md` T024 — its YAML cases referenced a bazel target name that never matched the BUILD declaration): executed 2026-06-17, produced `hasNativeReasoningBlock=false` and `additionalKwargs.reasoning_content="thinking..."` against `@langchain/openai` v1.4.7.
