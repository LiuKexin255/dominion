...
- [X] T001 Add additive `QueueSignal` message and `FlowPart` oneof field `QueueSignal queue = 9` (next free tag after `flow_result = 8`) to `projects/game/game.proto` per `specs/030-queued-chat-input/contracts/queue-channel-contract.md`; regenerate proto types via `bazel build //projects/game/game:game_proto` (or the repo's proto target) and run `bazel run //:gazelle projects/game` to refresh `BUILD.bazel`; verify `bazel build //projects/game/...` is green. No behavior wired yet.
...
- [X] T010 [P] [US4] Emit `QueueSignal` (proto field from T001) from `projects/game/agent/src/turn-loop.ts` on every per-session queue-depth change (submit ⇒ +1/new depth; drain-to-next-turn ⇒ 0; abort ⇒ 0) via the registered `emit` sink, per `specs/030-queued-chat-input/contracts/queue-channel-contract.md`. Add `QueueSignal` depth-change assertions to `projects/game/agent/src/turn-loop.test.ts`; run `bazel test`.
- [X] T011 [US4] Drive the pending-queue rendering from `QueueSignal.queued_count` in `projects/game/desktop/frontend/src/App.svelte` and `projects/game/desktop/frontend/src/components/ChatView.svelte`: replace the Phase-3 optimistic `queueCount` source with the backend `QueueSignal` value; transition pending messages to normal when depth decreases and the combined turn streams (spec FR-009). (Depends on T010.)
- [X] T012 [US4] Add a remove/cancel affordance for pending queued messages in `projects/game/desktop/frontend/src/components/ChatView.svelte` (spec FR-010): removing a pending message drops it before hand-off and the frontend reflects the decreased count. (Depends on T011.)
...
- [X] T014 Add queue-while-running large-test cases to the existing module test file `projects/game/testplan/agent_dialog_test.go` (organized by module per `style/large_test.md` — NOT by spec/feature number), and register the new cases as a `suite`/`case` in the existing `projects/game/testplan/system_test.yaml` (do NOT create a new per-feature YAML — `style/large_test.md` 反模式 #1/#4). Cover Scenarios 1-4 over the deployed agent SUT; follow `style/golang.md` (given/when/then, table-driven, no asserts inside cases). Run `bazel test //projects/game/testplan/...` to confirm the targets build/run.
- [X] T015 Execute the large test via the testplan skill: `guitar run projects/game/testplan/system_test.yaml` (full deploy→test→cleanup). Constitution §VI acceptance requires ALL cases to pass; fix and re-run until fully green. Build-only checks do NOT constitute acceptance.
