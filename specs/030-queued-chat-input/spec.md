...
- **FR-002**: Submitting a message while an agent turn is in progress MUST NOT interrupt, alter, split, or abort the in-flight turn. The in-flight turn MUST continue to its normal completion (the `wait` / turn-complete signal), identical to how it would run with no message queued. **[CLARIFIED — see Feature 038]**: [Feature 038 — Queued Input Mid-Turn Injection & Bubble Continuity](../038-queue-input-mid-turn/spec.md) narrows "alter" to *disruptive* alteration — interrupting in-flight tool execution (feature 038 [FR-012](../038-queue-input-mid-turn/spec.md)), splitting the agent's streaming text/thinking bubble (feature 038 FR-005/FR-006), or aborting the turn. The mid-turn injection of a queued message at a reasoning-step boundary (feature 038 FR-001 — the turn's first reasoning step or the reasoning step following a tool-result boundary) is a designed, bounded *content* change: it appends the queued message to the model's input before a reasoning step, without interrupting tool execution (FR-012), splitting the streamed bubble (FR-005/FR-006), or aborting the turn. The in-flight turn still runs to its normal completion, and the `wait` boundary remains the fallback hand-off point for no-tool turns and late arrivals (feature 038 FR-004).
