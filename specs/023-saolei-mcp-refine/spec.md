...
- Adapts [022 — Desktop Debug Mode](../022-desktop-debug-mode/spec.md): the tool-result hold and Confirm control are re-anchored onto the new content model (US4) — the hold stays at the same point (before the desktop returns the result to the agent), the Confirm control moves into a **session-top drawer** (C14/D11), and the control path and the conversation-render path are **fully decoupled** (C13/D10): the drawer associates with the held operation via the operation-channel id, not with any conversation bubble.
...
- **C6 (tool_id for bubble grouping)**: The `tool_call` MessagePart and the `tool_result` MessagePart share one `tool_id`, sourced from the LangChain tool_call id, so the conversation can group a call and its result into one evolving bubble. *(Revised by C13/D10: the `FlowPart` operation no longer shares this id — it uses a bridge-minted operation-channel id. Revised by C14/D11: the debug Confirm is no longer on the bubble — it is a session-top drawer.)*
...
- **C10 (debug hold re-anchor)**: The 022 hold stays anchored at the same point — the desktop holds after executing the operation and **before returning the result to the agent** (the control / `FlowPart` path). *(Revised by C14/D11: the Confirm control is in a **session-top drawer**, NOT on the conversation bubble; the control path and the conversation-render path are decoupled (C13/D10) and the drawer associates with the held operation via the operation-channel id, not via a shared `tool_id`.)* During the hold the execution outcome is visible in the log (C7); the conversation bubble shows the `tool_call` and updates with the `tool_result` only after the hold releases and the agent produces the tool's LLM result.
...
### Session 2026-07-25 (revision — decouple conversation from operation)

These clarifications revise the design after US3 implementation proved the MCP architecture cannot satisfy the original FR-008 coupling (an MCP tool handler cannot read `config.toolCall.id`, and the MCP adapter cannot carry `additional_kwargs`). See `research.md` D10/D11/D12 for the full root-cause and decisions.

- **C13 (decouple conversation channel from operation channel)**: The conversation display (tool_call/tool_result MessageParts) and the operation execution (FlowPart dispatch + debug hold) are **fully decoupled** and no longer share an id. The conversation groups tool_call↔tool_result by the LangChain `tool_call.id` (bubble grouping); the operation channel (`OperationBridge.dispatch`↔`ToolResultPart`) uses a **bridge-minted** id. This **relaxes FR-008**: the `tool_call` MessagePart and the `tool_result` MessagePart MUST still share the LangChain `tool_call.id` (bubble grouping); the `FlowPart` operation uses an independent bridge-minted id. `OperationBridge.dispatch` takes no `toolId` parameter (the bridge always mints). This lets MCP (saolei) tools dispatch without the LangChain id.
- **C14 (debug Confirm → session-top drawer)**: The debug Confirm control is **removed from the conversation bubble** and placed in a **drawer fixed at the top of the session chat view**, showing the operation request content (the FlowPart operation the desktop executed) + a Confirm button. This **supersedes FR-024** (control path no longer associates with the conversation render path via a shared id): the drawer associates with the held operation via the **operation-channel** id. The hold point (after execute, before return to agent), `SetDebugMode`/`ConfirmToolResult` methods, and `game:debug:result-held`/`result-released` event names are unchanged; the `result-held` payload gains an `operation` field. The conversation renderer no longer carries held state.
- **C15 (saolei/MCP tool status is neutral; real status only for native tools)**: The real `ToolResultStatus` is carried into the checkpoint (`ToolMessage.additional_kwargs.toolResultStatus`) **only for native tools** (mouse). **MCP** (saolei) tools return MCP content blocks; the adapter-built `ToolMessage` has no `additional_kwargs`, so their status is `TOOL_RESULT_STATUS_UNSPECIFIED` (neutral). This is **FR-014-compliant** (neutral, never FAILED) and **fixes the original bug** (item 3: saolei results showed spurious FAILED via text inference — now removed). The actual outcome remains visible via the result message text + screenshot. This **refines US2 acceptance** (FR-012/FR-013 apply to native tools; saolei reads neutral per FR-014).
...
After leaving and re-entering a session, every historical tool result shows the **same** success/failure status it had during the live turn. A **native** (mouse) tool operation that succeeded live still reads as succeeded in history; one that genuinely failed still reads as failed. The status is the actual outcome the desktop returned — never a guess from the result text, and never a default-to-failed. A result whose real status is not available is shown as neutral, not failed. **MCP (saolei) tool results show neutral (never FAILED) in history** (C15/D12) — the real status is carried only for native tools; for saolei the neutral default fixes the original spurious-"failed" bug.
...
### User Story 4 - Debug-Mode Hold Is Re-Anchored onto a Session-Top Drawer (Priority: P2)

With debug mode on, the desktop still holds each tool operation's result **after execution and before returning it to the agent**, and a **Confirm drawer at the top of the session chat view** shows the operation request content (the physical operation the desktop executed) plus a Confirm button. The control path (execute → hold → release → return) and the conversation-render path (tool_call bubble → later tool_result update) are **fully decoupled** (C13): the drawer associates with the held operation via the operation-channel id, not with any conversation bubble. During the hold, the execution outcome is visible in the log; the conversation bubble shows the tool call and updates with the result after the hold releases. Auto-continue (15 min) and the agent backstop (20 min) still apply.

**Why this priority**: This adapts the existing 022 behavior to the refactored content model. It depends on US1 (the new model); it is the lowest-priority story but is required so the debug experience is not broken by the refactor. The drawer design (C14) replaces the original Confirm-on-bubble design, which became impossible once the channels were decoupled (C13).

**Independent Test**: Toggle debug on; run a turn with a tool operation; confirm a drawer appears at the top of the session chat view showing the operation request + a Confirm button, and the agent does not advance until confirmed (or auto-continue); after confirm the drawer entry disappears and the tool bubble updates with the result; the execution detail is visible in the log during the hold.

**Acceptance Scenarios**:

1. **Given** debug mode on and a tool operation just executed, **When** the result would be returned to the agent, **Then** the desktop holds it and a **session-top drawer** appears showing the operation request content + a Confirm control; the agent does not advance.
2. **Given** a held result and the drawer's Confirm control, **When** the user clicks Confirm, **Then** the result is returned to the agent, the agent resumes, the drawer entry is removed, and the conversation bubble updates with the result.
3. **Given** a held result the user does not confirm, **When** 15 minutes elapse, **Then** the desktop auto-continues (returns the result, dismisses the drawer entry).
4. **Given** debug mode on, **When** an operation is held, **Then** the execution outcome is visible in the log panel during the hold.
5. **Given** debug mode off, **When** a tool operation runs, **Then** the result returns immediately with no hold and no drawer — identical to a non-debug run.
...
- **Debug toggled off mid-hold** / **leaving the session with a held result**: the held result is released (returned or torn down per existing 017/022 behavior); the drawer is not left with a dangling entry.
...
- **FR-008**: The `tool_call` MessagePart and the `tool_result` MessagePart MUST share one `tool_id`, sourced from the LangChain tool_call id, so bubble grouping works. *(Revised by C13/D10: the `FlowPart` operation now uses a **separate, bridge-minted** id — the conversation and operation channels are decoupled. `OperationBridge.dispatch` takes no `toolId` parameter.)*
...
- **FR-023**: With debug mode on, the desktop MUST hold a tool operation's result after execution and before returning it to the agent (the control / `FlowPart` path) — the same hold point as 022 — and surface the Confirm control in a **session-top drawer** showing the operation request content (not on a conversation bubble). *(Revised by C14/D11.)*
- **FR-024**: The control path (execute → hold → release → return) associates the Confirm drawer with the held operation via the **operation-channel** `tool_id` (bridge-minted); the conversation render path (tool_call bubble → tool_result update) is **independent** and is NOT relied upon for the association. *(Supersedes the original FR-024 — revised by C14/D11.)*
- **FR-025**: Clicking Confirm (in the session-top drawer) MUST release the held result to the agent, dismiss the drawer entry, and (once the agent produces the tool result) update the conversation bubble with the result. *(Revised by C14/D11.)*
...
- **ToolCallPart (new)**: the display representation of a model tool invocation — tool name, input arguments, and a `tool_id` linking it to its `tool_result` **within the conversation channel** (the `FlowPart` operation uses a separate, bridge-minted id per C13/D10).
...
- **Evolving tool bubble**: one conversation bubble per tool call, identified by the conversation-channel `tool_call.id`, that shows the call first and is updated in place with the result when it arrives.
...
- **Debug hold (re-anchored)**: the desktop's pre-return hold of an operation result (control path), with its Confirm control in a **session-top drawer** (C14/D11), associated to the held operation via the **operation-channel** id (decoupled from the conversation per C13/D10).
...
- **SC-003**: In 100% of reconnect cases, a **native** tool operation that succeeded live shows as succeeded after re-entering the session; one that genuinely failed shows as failed; an unknown status shows as neutral. **MCP (saolei)** tool results show neutral (never FAILED) on re-entry (C15/D12). Net: zero spurious "failed" results from history replay.
...
- **SC-005**: In 100% of debug-mode holds, a session-top drawer appears showing the operation request + a Confirm control (associated via the operation-channel id), the agent does not advance until confirmed or auto-continued, and the execution outcome is visible in the log during the hold.
...
- The mouse tools already include the post-action screenshot in their LLM result via `buildResultBlock` (`projects/game/agent/src/tools/shared/result-blocks.ts`); the saolei tools today do via `resultFromDispatch` (US3 rewrites them to return MCP content blocks — text + image — instead). So FR-010 (screenshots from the LLM tool result) is satisfied at the source for both tool families; the work is to render that source consistently and stop the desktop-side mirror-for-display.
...
- `projects/game/agent/src/operation-bridge.ts` — `dispatch` (writes the operation Part as a control frame; mints its own **operation-channel** `tool_id` — decoupled from the conversation `tool_call.id` per FR-008 revised / C13/D10), `handleResult` (where the real status is resolved), `pushResult`.
- `projects/game/agent/src/tools/shared/result-blocks.ts` — `buildResultBlocks` (writes `result.message` + screenshot into the `ToolMessage`); the real status is now carried for native tools via `buildToolResultMessage` (`additional_kwargs.toolResultStatus`, US2/Phase 3); saolei (MCP) tools remain neutral (D12).
