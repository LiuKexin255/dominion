...
- **端到端冒烟（②，已完成，2026-08-03）**：deploy service 新版本（含 `EnvironmentStatus.services`，镜像 digest `sha256:1c7efb0...`，见 `projects/infra/deploy/k8s.yaml`）已上线 infra.liukexin.com，按 quickstart.md 场景 A/B 完成验证：
  - **场景 A（部署不成功 + per-service 状态主线）**：临时修改 `experimental/ts/grpc_hello_world/src/bootstrap.ts` 令 `service` artifact 启动即 `process.exit(1)`（CrashLoopBackOff），`gateway` 保持健康；`guitar run ... --timeout=60s` → deploy apply 超时被杀 → 打印 `  --- 环境状态 (env=liukexin.ltd8hlts) ---` 头部 + 顶格 describe：`状态: 等待滚动发布`、服务列表**内联 per-service 状态**——`service ... [artifact] 等待发布: Deployment ... 可用副本不足（available: 0/1）`（WAITING，崩溃中的服务）、`gateway ... [artifact] 就绪`（READY，健康网关）；**无 `说明:` 行**（有 per-service 数据时按契约不输出，决策 R5 生效）；原始错误保留 `deploy apply ...: signal: killed`；cleanup 执行（env 已删除）。满足 FR-001/002/003/004/006/007/010、SC-002/SC-003/SC-004。
  - **R4 时序空窗消除验证**：controlled 轮询 `GetEnvironment` 原始 API，进入 `WAITING_ROLLOUT` 后 t=5s 即见两服务均为 `PENDING`（`资源已提交，等待观测`，由 `applyAndWait` 写入）；t=10s 起 `service`→`WAITING`、`gateway`→`READY`。证明进入 WAITING_ROLLOUT 即有 per-service 数据，短超时亦能列出服务（决策 R4）。
  - **边界：非 TTY 无 ANSI（FR-009）**：场景 A 输出经 `tee` 管道，grep `\x1b[` 无命中。
  - **场景 B（成功路径回归，FR-008/SC-005）**：回退 bootstrap.ts 改动，`guitar run ... --timeout=120s` → deploy 成功（就绪）→ bazel test PASS → cleanup；**无** `--- 环境状态 ---` 头部、**不**调 describe（成功路径不进诊断分支）。
  - **FAILED 终态（SC-001）**：crash-loop 在 60s 窗口内表现为 WAITING（可用副本不足），未达 `ProgressDeadlineExceeded`（k8s 默认 ~600s）；WAITING + per-service 原因已充分验证"哪个 service 出问题"的诊断目标，FAILED 渲染由单测 `TestDescribeCommand` 与 `TestCheckRollout` 覆盖。
  - **环境清理**：所有临时 env（probe/p032svc/w032a/ltdtwsxy/ltrhfgj8/ltd8hlts/lt5bk9o1）均已 `deploy del` 删除；bootstrap.ts 临时改动已回退（`git diff` 干净）。
  - **前置修正**：执行中发现首跑 describe 仍输出初版格式——根因是已安装的 `deploy` 二进制为旧版（无 Phase 5 per-service 改动），guitar shell-out 消费之；执行 `bazel run //:deploy_install //:guitar_install` 重装新版二进制后输出正确。后续执行 testplan 前须确保工具已重装。
