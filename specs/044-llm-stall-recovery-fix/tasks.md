...
- **官方文档**: 无
- **技术文章**: [survey](../../survey/llm-stream-stall-recovery-revision.md) §7.2（验证门禁）；[research.md](research.md) R4/R7/R8；[contracts/desktop-rendering-contract.md](contracts/desktop-rendering-contract.md) §1/§2（WarnSignal ⚠ 气泡 + proto 调和）；[quickstart.md](quickstart.md)（A4 大型测试取消说明 / B6 / C3 验收场景）
...
- [ ] T011 Large-test cases (by module, per `style/large_test.md`): extend `projects/game/testplan/agent_stall_test.go` (the stall module) with focused cases — (a) **US2 — 大型测试用例取消（在该部署下不可测）**。原因：stall 部署（`deploy_agent_stall.yaml`）显式设置 `GAME_STREAM_IDLE_TIMEOUT_MS`（重基线后 `"60000"`），按 [idle-timeout-contract.md §1](contracts/idle-timeout-contract.md#1-resolution-rule) + FR-003/US2.3，显式 env 始终 as-is 生效、floor 被完全抑制（floor 仅在 env 未设置时以 `max(default, floor)` 抬升有效超时）。因此 deepseek 的 600s floor 在此部署根本不生效——model spec 对可观测结果零影响（无论是否传 deepseek spec，explicit env 都使有效超时 = 60s），任何以 deepseek spec 替换 gpt-4 的变体都与既有 stall 用例可观测行为完全相同（duplicate），既检不出 floor 回归也检不出接线回归。US2 floor 的**解析**（`resolveStreamIdleTimeout` / `getReasoningIdleTimeoutFloor`）与 **graph-node 应用**（`nodes.player.timeout.idleTimeout`）已由 T003（`reasoning-timeouts.test.ts`，含 env-below-floor as-is 分支）+ T004（`graph.test.ts`：deepseek+env-unset → `=== 600_000`；deepseek+env-explicit-90s → `=== 90_000`）在**单测层完整验证**；floor 的运行时效果（LangGraph 按解析出的 `idleTimeout` 计时）是 LangGraph 契约、非本仓库逻辑。**不新增 env-未设置的部署变体（option B 已否决）**：需新部署拓扑 + 每用例 >120s 静默观测来测 LangGraph 计时器（非 dominion 逻辑），且 fake-llm 只有永久 stall（`sample_stall.yaml` `stall:true`，无“静默 N 秒后恢复”模板），即使建了 env-unset 部署也无法观测 floor 的“容忍后恢复”收益（永久 stall 在 600s floor 下也会 fire）——成本高、价值低，与 `style/large_test.md` §反模式（避免为单一特性 proliferate 部署/计划）相悖。**option C（重定义为“显式 env 下 deepseek 仍被检测”）亦否决**：deepseek spec 在显式 env 下不改变任何可观测行为，是既有 gpt-4 stall 测试的纯 duplicate。US2 的 floor 是 build-time 计算；其运行时 stall 检测行为与 US1 共享、由本 suite 既有 `TestAgentStallRecoveryWithQueuedMessage` + 下文 case (b) 覆盖——Constitution VI「覆盖关键服务行为」由此满足。**SC-005 的大型测试组件在此拓扑下不可满足**（见 quickstart.md A4 与 spec owner 裁决反馈——勿自行改 spec.md）。 (b) **US1 regression**: a genuine silent dropout is still detected within the configured window (deploy env 60s, FR-001); (c) **US3**: stall mid-reply → re-enter the session → `ListMessages` returns the partial output with the interrupted marker. **Case (c) assertion** (now that interrupted is a formal proto field): assert the partial reply's tail content part carries `completion == game.PartCompletion_PART_COMPLETION_INTERRUPTED` — read via `part.GetText().GetCompletion()` (text) or `part.GetThinking().GetCompletion()` (reasoning) on the `ListMessages` response `Message.content.parts`. A normal (non-interrupted) part has `completion == PART_COMPLETION_UNSPECIFIED` (the protojson default — field absent). **重基线 stall suite 到 FR-001 的 60s 新下限**（现部署 env `15000` 低于新下限，会被 T002 的钳制静默改成 120s，破坏既有用例时序）：`projects/game/testplan/deploy_agent_stall.yaml` env `GAME_STREAM_IDLE_TIMEOUT_MS: "15000"` → `"60000"`（并更新 `desc`）；相应调整 `agent_stall_test.go` 时序假设 —— `stallToolReplyDelay` 20s → >60s（如 65s，保持 heartbeat 验证有效）、`helpers_test.go:35` `wsReadTimeout` 30s 与 `drainUntilWait` 预算 → >60s（如 75s），并刷新 `agent_stall_test.go`/`system_test.yaml` suite 11 描述中的 15s 窗口注释；suite 11 描述应反映 044 的实际大型用例为 **case (b) US1 真实静默回归检测 + case (c) US3 partial-output 持久化**（US2 floor 无大型用例，见上文 case (a) 取消依据）。用例经既有 `system_test.yaml` 的 **agent-stall suite**（其 `deploy` 即 `deploy_agent_stall.yaml`）运行——不要新建 per-feature YAML（`style/large_test.md` §测试计划数量/§反模式），也不要把 suite/case 写进 deploy-only 的 `deploy_agent_stall.yaml`（它没有 `suites` 段）。Reuse `helpers_test.go`/`saolei_fixtures_test.go`; follow `style/golang.md`. Run `bazel build //projects/game/testplan:...` on the test target.
