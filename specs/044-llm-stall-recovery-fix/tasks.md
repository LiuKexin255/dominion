...
**T012 首跑修订（2026-08-14，新增 T021）**: T012 首轮 `guitar run` agent-stall suite 5 用例 4 过 1 败——case (d) `TestAgentStallThinkInterruptGapDetected` 失败（trace `fbac92826519a78413780294766e772b`：收到的首个 thinking frame 是 `greeting` 模板 reasoning "The user is greeting me, I should respond warmly."，第二个 thinking frame 75s 读超时）。**根因（fake-llm 关键字碰撞，非 agent 侧回归）**：匹配器 `projects/game/fake-llm/service/matcher.go:26-44` 按 [012 spec](../012-fake-llm-service/spec.md) FR-006/FR-007 契约语义工作——关键字为用户文本的大小写不敏感**子串**即命中、多命中取 Name 字典序最小者；用例文本 `"think interrupt gap"` 同时命中 `greeting`（关键字 `hi` ⊂ "t(hi)nk"，`projects/game/fake-llm/service/testdata/chat.yaml`）与 `think-interrupt-gap`（完整关键字，`testdata/stall_recovery.yaml`），字典序 `"greeting" < "think-interrupt-gap"` 使 greeting **确定性**胜出（matched=true，连 warn 日志都不打）。机械枚举 7 个 testdata 文件全部关键字：与该文本碰撞的仅此两模板；且 **046 全部三个 think 模板的关键字都含 "think"（内嵌 "hi"）**——`think-interrupt-stall`/`think-healthy-cadence` 在合并存储中同样被 greeting 劫持而不可触发（仅因无大型测试消费者而未暴露，T020 若采用含 "think" 的触发词也会踩中）。修复采用 **greeting 关键字卫生**（移除 `hi`，落在碰撞根源一侧）：新增 **T021**（chat.yaml + `hi` 的唯一消费者 `TestAgentDialogFIFOQueue` + testplan/fake-llm README 措辞同步）；`matcher.go`/`matcher_test.go` 零改动（012 FR-006/FR-007 契约语义与既有 pin 保持）；case (d) 测试文本与 046 T012 钉死的 `think-interrupt-gap` 关键字**均不变**。执行顺序更新为 **T014–T017 → T011 ∥ T018 ∥ T019 ∥ T021 → T012 → T013（T020 optional）**——见 Dependencies。046 契约示例的同步修订（[template-config.md](../046-fake-llm-think-chunking/contracts/template-config.md) §3.1/§3.6 greeting 示例 + cross-feature note）已随本修订落地。
...
- [x] T018 [P] Update `projects/game/testplan/system_test.yaml`: §11 注释块（`:76-80` 附近）与 `agent-stall` suite description（`:194-195`）中 "GAME_STREAM_IDLE_TIMEOUT_MS=60000 / deploy_agent_stall.yaml env" 表述全部改为 config 表述（"deploy_agent_stall.yaml selects the agent_timeouts config block (streamIdleTimeoutMs=5000, toolHeartbeatIntervalMs=2000) via 045 deploy-config; the agent resolves env > config > default"）；若 description 提及心跳 10s 处改 2s。同步更新 `projects/game/testplan/helpers_test.go:35-42` 的 `wsReadTimeout` 注释（"deploy_agent_stall.yaml GAME_STREAM_IDLE_TIMEOUT_MS=60000" → config 驱动表述）；`wsReadTimeout = 75s` 常量值不改（读超时上限，仍远超 5s 窗口）。仅注释/描述，不改 suites/cases 结构与常量。**2026-08-14 amendment（executor 执行前校验发现规划遗漏：fd81521 的 60s 重基线共写入四处的 env/60s 表述——system_test.yaml、helpers_test.go、README.md、BUILD.bazel，原 T018 只列前两处）——文件范围扩展为四处，新增下述 (3)/(4)；表述基准不变：T011 已落地的 `projects/game/testplan/agent_stall_test.go:50-57` 头注释与 `system_test.yaml:76-82` §11 注释，不引入第三种措辞**：
  ...
  验证（新增范围）：`bazel build //projects/game/testplan:agent_stall_test`（BUILD 文件与 size 属性合法性）；无 yaml/常量改动，`guitar validate` 与单测不受影响；档位的实际印证在 T012——全绿运行观测时长应 ≈60–90s，远低于 medium 的 300s。 **(done — commit `c4aae69`；docs amendment `e929b60`)**
- [x] T019 [P] fake-llm hygiene — **2026-08-14 amendment（executor 前置校验中止后的事实更正）**: 原文声称 "pin 断言已核实无需同步"，与代码不符——046 store-level pin 测试**确实 pin 了** `chunk_delays`。变更共两个文件：(1) `projects/game/fake-llm/service/testdata/stall_recovery.yaml` — `think-interrupt-gap` 的 `chunk_delays: ["1s", "90s"]`（`:43`）→ `["1s", "15s"]`（5s 窗口下检测在 gap 起始 ~5s 触发；15s 保证 gap 覆盖检测点，且用例失败路径（如检测未触发）最多多等 15s 而非 90s）；同步更新模板注释两处 "90s" 表述——`:29` "90s gap (above the agent's idle timeout …)" 改为 15s，并将 "above the agent's idle timeout" 措辞改为指向 stall 部署 config 驱动的 5s idle 窗口（15s 仅在该窗口下超过 idle；默认拓扑 120s 下不触发——模板由显式关键字命中（T011 case (d)）且被 FR-011 排除出随机 fallback），`:33` "The 90s chunk_delays entry" → "The 15s …"（仍为非零 delay，FR-011 isHangCapable 排除语义不变）；契约引用行（`:26-27` → [046 template-config.md §3.3](../046-fake-llm-think-chunking/contracts/template-config.md)）**不改**——契约示例保留 90s 原形状，其 "044 cross-feature note (2026-08-14)" 已记载 rescale。(2) `projects/game/fake-llm/service/message_store_test.go:819-821` — `TestNewMessageStore_LoadsEmbeddedSamples`（`:507`）的 pin 断言 `slices.Equal(thinkGap.ChunkDelays, []string{"1s", "90s"})` 同步改为 `["1s", "15s"]`（`:820` 错误信息中的 want 字面量 `[1s 90s]` 一并更新）；同函数其余 delay pin 不受影响（`think-healthy-cadence` `["200ms","200ms"]` `:802-804`、`think-interrupt-stall` `["1s"]` `:836-838`）；该测试注释块（`:789-794`）与 `projects/game/fake-llm/service/handler_test.go` 均无 "90s" 字面量（grep 核实），无需改动。验证：`bazel run //:go -- fmt projects/game/fake-llm/service/message_store_test.go`（本任务新增 Go 变更）+ `bazel test //projects/game/fake-llm/...` 全绿（含 pin 测试——原文 "本任务仅改 testdata" 与回归绿要求内部矛盾，本修订消除）。**"90s→15s" 跨文档记录经复核仍然如实、无需改动**：[quickstart.md](quickstart.md) D3、[research.md](research.md) R11、[large-test-status.md](large-test-status.md) §6 046 条目、[046 template-config.md §3.3](../046-fake-llm-think-chunking/contracts/template-config.md) cross-feature note 均只记载 "T019 将 90s 缩至 15s"（rescale 事实），不含 "pin 无需同步" 的错误声明；046 template-config.md 自身契约语义（字段规则/示例形状/校验规则）不受本修订影响，不作改动。 **(done — commit `c4aae69`；docs amendment `2328861`)**
- [x] T021 [P] fake-llm keyword-collision fix（greeting 关键字卫生——T012 首跑修订，2026-08-14）: 根因与裁决见本文件 header "T012 首跑修订"——`greeting` 的 2 字符关键字 `hi` 是 "t(hi)nk" 的子串，按 [012 spec](../012-fake-llm-service/spec.md) FR-006/FR-007（子串命中 + Name 字典序 tie-break）在合并存储中确定性劫持 046 **全部三个** think 模板的触发文本（`think-interrupt-gap`/`think-interrupt-stall`/`think-healthy-cadence` 关键字均含 "think"），T012 case (d) 因此失败。修复落在碰撞根源（greeting 侧）：**`think-interrupt-gap` 的关键字、case (d) 的测试文本、`matcher.go`/`matcher_test.go` 均不变**。变更四个文件：(1) `projects/game/fake-llm/service/testdata/chat.yaml` — `greeting` 的 `keywords` 删除 `- hi` 条目（保留 `hello`/`greetings` 两个关键字，满足 012 FR-004/FR-009 的 ≥1 非空约束），并在 greeting 条目上方加简短注释记录原因（"hi" ⊂ "t(hi)nk" 使 greeting 按字典序劫持所有 think-* 触发文本；依据 specs/044-llm-stall-recovery-fix/tasks.md T021；语义见 specs/012-fake-llm-service/spec.md FR-006/FR-007）。(2) `projects/game/testplan/agent_dialog_test.go:249-253`（TestAgentDialogFIFOQueue——全仓 `hi` 关键字的唯一消费者，grep 核实）— `messages` 第三条 `"hi friend"` → `"greetings friend"`，注释 `// greeting again (hi is a greeting keyword)` → `// greeting again (greetings is a greeting keyword)`（FIFO 身份证明语义不变：仍命中 greeting、仍证明多关键字数组可触发，仅换用第三关键字 "greetings"；"greetings friend" 不含任何其他模板的关键字，无新碰撞）。(3) `projects/game/testplan/README.md` — `:52-55` 数据文件格式示例删除 `  - hi` 行；`:79` shipped 表 greeting 行 keywords 列 `hello, hi, greetings` → `hello, greetings`；`:64-66` `keywords` 字段 bullet 末尾补一句作者提示（过短的关键字是子串碰撞源——例如曾经的 2 字符 "hi" 会匹配 "t(hi)nk"；关键字应足够长且不得为其他模板关键字的子串）。(4) `projects/game/fake-llm/README.md:15-17` Key capabilities 第一条更正匹配语义措辞：`whose \`keywords\` all occur (case-insensitive substring) in the user/assistant turn` → `where ANY of its \`keywords\` occurs (case-insensitive substring) in the last \`user\` message`（实现为 `matcher.go` `anyKeywordMatches` + `handler.go` `lastUserText`——原文 "all" 与 "user/assistant" 均与实现不符；本次匹配语义事故的邻近文档更正）。**同步记录（已随本修订由 designer 落地，勿重复改）**：[046 contracts/template-config.md](../046-fake-llm-think-chunking/contracts/template-config.md) §3.1/§3.6 的 greeting 示例 keywords 已同步为 `[hello, greetings]`，§3.1 已加 "044 cross-feature note (2026-08-14, keyword hygiene)"。**验证**：`bazel run //:go -- fmt projects/game/testplan/agent_dialog_test.go` + `bazel test //projects/game/fake-llm/...` 全绿（`message_store_test.go:587` 的 greeting pin 仅断言含 `hello`、`:515` 计数 17、sorted-name 列表均不受影响——已核实无任何 pin 断言 "hi"）+ `bazel build //projects/game/testplan:agent_dialog_test //projects/game/testplan:agent_stall_test`；agent-dialog 套件的端到端绿灯由 T012 的全量 `guitar run` 覆盖（guitar run 执行整个 `system_test.yaml`，含 agent-dialog suite 的 FIFO 用例）。 **(done — commit `a84a6a2`；designer ruling docs `bd3c47a`)**
- [x] T012 Large-test acceptance (Constitution VI): execute the testplan via the testplan skill — full deploy→test→cleanup loop: `guitar validate projects/game/testplan/system_test.yaml && guitar run projects/game/testplan/system_test.yaml`（agent-stall suite：043 二用例 + 044 (b)(c)(d) 三用例 = 五用例，config 驱动 5s/2s 时序下全套预计 <2min）。**All cases MUST pass**（任何 failed/flaky = 验收未过，修复后重跑直至全绿；build-only 不构成验收）。**SC-005 裁决收口（spec owner，research.md R11）**：验收记录 MUST 同时载明 α/β/γ 裁决结果——α 或 β 时 SC-001/SC-005 以单测替代并记录在案，γ 时按 T020 补 floor 大型用例。**R9 应急分支（仅当心跳用例 `TestAgentStallToolExecutionNotFalselyDetected` 再失败时执行）**：用 signoz skill 查询本次运行 trace/logs（service `game/agent`）——per-tick 日志在且 false stall → LangGraph `touch()`→`checkIdle` 路径问题（升级调查至 LangGraph 层，考虑 refreshOn/配置绕行）；tick 日志缺失/停止 → wrapper 定时器生命周期 bug（`llm.ts` `withIdleHeartbeat`，修 `setInterval` 路径）；定位后修复、重跑直至全绿。**首跑记录（2026-08-14，T021 依据）**：首轮 `guitar run` 4/5——case (d) 失败于 fake-llm 关键字碰撞（`greeting` 的 `hi` ⊂ "t(hi)nk" 字典序劫持 `think-interrupt-gap`；根因与机械枚举见本文件 header "T012 首跑修订"），**非 agent 侧回归**（trace `fbac92826519a78413780294766e772b`）。T021 落地后重跑整个 system_test.yaml：agent-stall 五用例全绿 **且** agent-dialog suite（FIFO 用例换用 "greetings friend" 触发词）全绿，方为验收通过。 **(done — 全绿验收：首轮 4/5 → T021 修复 → 重跑 11/11 suites 全绿（agent-stall 五用例 5.35s/12.31s/5.17s/5.09s/5.10s，suite 33.1s）+ agent-stall 单独复跑全绿 33.0s；无代码 commit——运行验收，详见 [large-test-status.md](large-test-status.md) §7)**
- [x] T013 Housekeeping: `bazel run //:gazelle projects/game/agent/src`（T014 新文件 target）与 `bazel run //:gazelle projects/game/testplan`；`bazel run //:go -- fmt`（T011/T019 的 Go 改动已在各自任务内 fmt——T011 `agent_stall_test.go`、T019 `message_store_test.go`（2026-08-14 amendment 新增）；yaml 无需 fmt，仅当另有 Go 改动）；`bazel mod tidy`；最终 `bazel build //...` + `bazel test //projects/game/agent/...`（+ `bazel build //projects/game/desktop/frontend:dist`）全树绿；将 T012 全绿结果与根因裁定结论（心跳用例是否复发、tick 日志证据）以及 SC-005 α/β/γ 裁决结果（spec owner）追加到 [large-test-status.md](large-test-status.md)（§1 进度表与 §4 裁决栏）；勾选本文件全部任务。 **(done — housekeeping 全绿：gazelle×2 / go fmt×4 幂等 / `bazel mod tidy` / `bazel build //...` / `bazel test //projects/game/agent/...` / `bazel build //projects/game/desktop/frontend:dist` 全部通过，工作树无未提交变更；文档收口见 [large-test-status.md](large-test-status.md) §7；无代码 commit——本任务为验证与文档操作，由 executor 统一提交)**
- [ ] T020 **[OPTIONAL — gated on SC-005 ruling (research.md R11; [large-test-status.md](large-test-status.md) §4)]** γ floor large-test case（spec owner 裁决为 γ 才执行，默认 α 不做）：在标准套件既有模块文件（`projects/game/testplan/agent_saolei_test.go`，deploy `deploy_agent.yaml`——默认拓扑 env+config 均未设 → 有效超时 `max(120s, deepseek floor 600s)`）新增 ~3min 用例：fake-llm 新增 resumable 模板 `think-floor-tolerance`（`reasoning_chunks` 3 块、`chunk_delays: ["1s", "150s"]`，>120s 默认且 <600s floor；**触发关键字按 Notes 的选词纪律做碰撞自查**——T021 后 "think" 前缀无碰撞，但不得引入新的短关键字子串）；deepseek profile（`setupTeamSession(..., "deepseek-v4-flash", "deepseek-v4-flash")`——profile 模型名即 bare model spec，经 `team/graph.ts` 传入 `resolveStreamIdleTimeout`；`deepseek-v4-` 子串命中 allowlist（`projects/game/agent/src/reasoning-timeouts.ts:36-38`），与 T003 单测的完整 spec 形式 `openai/deepseek-v4-flash` 等价命中）；断言 150s 静默 gap 内**无** warn/wait frame、gap 后 chunk2+text 到达、turn 正常终止（恰 1 个 terminal wait）——证明 floor 抬升生效（回归时 120s 默认会在 gap 中触发 false stall）。执行 `guitar run projects/game/testplan/system_test.yaml`（saolei suite）全绿。 **（not executed — SC-005 working default α; ruling pending spec owner）**

---

## Dependencies & Execution Order

> **编号与执行顺序说明**：T001–T013 编号因被 plan.md / large-test-status.md / quickstart.md 引用而保持稳定；**实际执行顺序为 T014–T017（Phase 5A）→ T011 ∥ T018 ∥ T019 ∥ T021（Phase 5）→ T012 → T013（→ T020 可选）**（T021 为 2026-08-14 T012 首跑修订新增——见 header）。已完成任务（`[x]` T001–T010）不重做。
...
- **Phase 5 剩余**: T011/T018/T019/T021 依赖 Phase 5A 完成（时序由 config 决定；T021 为 T012 首跑修订新增——fake-llm 关键字碰撞修复，见 header "T012 首跑修订"）；四者不同文件可并行。T012 依赖 T011+T018+T019+T021 全部落地。T013 收尾最后。T020 仅在 SC-005 裁决为 γ 后执行（依赖 Phase 5A 的反向验证：γ 需默认路径，与 T011 互不依赖）。
...
- Phase 5 内：T011/T018/T019/T021 并行（不同文件）；T012 是唯一验收门禁（Constitution VI）；T013 收尾。
- 单测随各实现任务内联（Constitution IV），不单列 task。

### Parallel Opportunities

- T011 ∥ T018 ∥ T019 ∥ T021 —— 不同文件（`agent_stall_test.go` / `system_test.yaml` + `README.md` + `BUILD.bazel`（2026-08-14 amendment 扩展）/ fake-llm `stall_recovery.yaml` + `message_store_test.go` pin 断言（2026-08-14 amendment）/ fake-llm `chat.yaml` + `agent_dialog_test.go` + `testplan/README.md` + `fake-llm/README.md`（2026-08-14 T012 首跑修订新增）），Phase 5A 完成后可并行。
...
Task: "T021 fake-llm greeting 关键字卫生（chat.yaml 移除 hi + agent_dialog_test.go FIFO 触发词换 greetings + 两处 README 同步）——2026-08-14 T012 首跑修订新增"

# 全部落地后:
Task: "T012 guitar run 大型测试验收（全绿闭环，含 R9 应急分支预案；首跑 4/5 的 case (d) 失败已由 T021 修复，重跑全量）"
...
- **fake-llm 新模板关键字选词纪律**（T021 根因教训，2026-08-14）：匹配是"关键字 ⊆ 用户文本"的**子串**语义且多命中按 Name 字典序取最小（[012 spec](../012-fake-llm-service/spec.md) FR-006/FR-007）——新模板的关键字必须足够长，且**不得是任何其他模板关键字的子串**、也不应内嵌其他模板的短关键字（曾经的 `hi` ⊂ "t(hi)nk" 使 greeting 劫持全部 think-* 触发）。T020 的 `think-floor-tolerance` 触发词同样受此约束（T021 后 "think" 前缀已无碰撞，但仍需按 testplan README §3 的提示做碰撞自查）。
