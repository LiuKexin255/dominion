...
The content block that was mid-stream at the stall (typically the last block of the partial) carries `additional_kwargs.interrupted = true` **in the checkpoint layer** (the agent's in-process AIMessage content array — this survives the `MemorySaver` serde and is consistent with how `toolResultStatus` is carried on `additional_kwargs` at `llm.ts:428-435`). **Only that block is marked** — earlier completed blocks in the same partial are unmarked (spec FR-005, Session 2026-08-12 clarification). The precise rule for determining "mid-stream" (including the tool-gap case) is defined in §3 "Marking rule".

**Two-layer distinction (critical for implementation)**:

- **Checkpoint layer** (agent process, in-memory AIMessage): the flag is `additional_kwargs.interrupted = true` on the content-block array element. This is what `mergePartialBlocks` writes and what survives the `MemorySaver` serde. It is the **input** to the ListMessages translation. This layer keeps the `additional_kwargs` mechanism (proven feasible in the R4 spike and T006 tests — it never leaves the agent process).
- **Wire layer** (proto, cross-network): the flag is the **proto enum field** `TextPart.completion`/`ThinkingPart.completion` = `PART_COMPLETION_INTERRUPTED` (`projects/game/game.proto`, field number 2 on each). This is what `ListMessages` reconstruction **translates** `additional_kwargs.interrupted` into, and what crosses the network to the desktop.

**Why two layers**: the checkpoint stores LangChain `AIMessage` objects (the `additional_kwargs` shape); the wire speaks proto (`MessagePart`). A loose-JSON passthrough (extra `interrupted` field) was proven infeasible — every network hop (proto-loader serialize, grpc-go, grpc-gateway, desktop `DiscardUnknown` unmarshal, strict `protojson.Marshal`) strips unknown fields. The marker must be a **declared proto field** to cross the network. The translation `additional_kwargs.interrupted → PartCompletion.completion` happens once, in `handler.ts` ListMessages reconstruction.

`ListMessages` reconstruction (`handler.ts:804-847`) translates the checkpoint-layer flag into the proto field: when emitting a `TextPart`/`ThinkingPart` for a content block carrying `additional_kwargs.interrupted`, it sets `completion = PART_COMPLETION_INTERRUPTED` on the emitted part; a normal complete AIMessage emits parts with the default `PART_COMPLETION_UNSPECIFIED` (protojson omits the zero value). The desktop renders the `completion` field (desktop-rendering-contract.md §3).
...
- Unit (`handler.test.ts`): `ListMessages` returns the partial AIMessage with the interrupted marker translated onto the flagged part's proto `completion` field (`PART_COMPLETION_INTERRUPTED`); normal AIMessage parts carry the default (no `completion` field / `UNSPECIFIED`).
- Large test: stall mid-reply → reconnect → `ListMessages` returns the partial output (spec SC-002/SC-003). The Go assertion reads `part.GetText().GetCompletion()` / `part.GetThinking().GetCompletion()`.
