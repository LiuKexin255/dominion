...
**Decision**: Carry the "interrupted" marker as a property on the **content block itself** in a field the reconstruction passes through, AND make the partial AIMessage's content a **single trailing block** of the interrupted kind. Concretely:

- The merged AIMessage's `content` is an array. The interrupted tail block carries an extra field, e.g. `{ type: "text", text: "…", additional_kwargs: { interrupted: true } }` (or `type: "reasoning"`). The exact key is finalized in tasks.md (candidate: `additional_kwargs.interrupted = true` on the block, consistent with how `toolResultStatus` is already carried on `additional_kwargs` at `llm.ts:428-435`).
- `handler.ts` reconstruction is extended minimally: when emitting the part for a content block that carries `additional_kwargs.interrupted`, propagate an `interrupted: true` marker onto the emitted `MessagePart` (text or thinking). This is a **small, additive change** to the reconstruction (it already special-cases reasoning/text/image filtering).

**Why per-block is achievable here**: in practice the partial AIMessage has at most `[reasoning…][text…]` and the interrupted one is the **last** block (the one being streamed when the stall hit). Because the reconstruction already separates reasoning from text, marking the emitted text-part (or thinking-part) as interrupted maps cleanly to "the text/thinking that was cut off." If both reasoning and text were streaming (rare), the LAST emitted block type is marked.
...
- **A new `MessagePart` kind** (e.g. `interrupted_marker`): rejected — requires a proto change (out of scope) and pollutes the content model.
- **Drop per-block, mark whole message**: rejected (contradicts FR-005/SC-003 as clarified).

**Verification gate**: `handler.test.ts` — a checkpointed AIMessage whose last content block carries the interrupted flag is returned by `ListMessages` with the corresponding `MessagePart` marked interrupted; a normal complete AIMessage has no such marker.
...
**Decision (FR-013)**: The desktop history-seed path (which builds chat entries from `ListMessages` on reconnect) is extended to render the `interrupted` marker (R5) on the flagged `MessagePart` — e.g., a visual "中断"/truncated indicator on that bubble. The live stall notice continues to use the ⚠ warn bubble (transient, live-only); the interrupted flag is what survives reconnect.
...
| Interrupted-flag carrying | Per-content-block `additional_kwargs.interrupted`, reconstruction passes it through | R5 |
