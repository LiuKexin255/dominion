...
| A3 | 产物中存在内容哈希命名的 JS 资源（`assets/*-<hash>.js` 形态，hash 为十六进制段） | FR-001 |
...
5. A4 的 React 运行时痕迹字面量 `react-dom.production.min.js` 依赖 vite 默认 license 注释保留行为：T010 实现时以实际产物核对该字面量确实存在；若未来 minify/legalComments 配置变更导致该 banner 消失，实现时以产物中可稳定检出的 React 字面量替换，并同步更新本契约与 `dist_assert.sh`（单一来源）。
