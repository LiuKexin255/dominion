...
**Independent Test**: `bazel test //projects/game/web/frontend/...`——FR-022 消费覆盖断言、Menu 卡片视觉断言与既有组件用例全绿；`bazel build //projects/game/web/frontend:dist` 通过；人工浏览器目验菜单弹出形态
...
- **技术文章/技术参考文档**：`specs/054-agent-v2-bugfixes/contracts/web-ui.md` §1/§7、`specs/054-agent-v2-bugfixes/research.md` D4（含载体重设计修正注）、`specs/054-agent-v2-bugfixes/revisions/phase10-theme-css-carrier.md`（T020 权威依据：载体裁定与终态设计 §3、溯源与人工升级流程 §3.2、FR-022 覆盖断言 §3.6、测试义务 §4、返工处置 §6）

- [x] T020 [US8] token CSS 引入与 Menu 视觉修复（依据 `specs/054-agent-v2-bugfixes/revisions/phase10-theme-css-carrier.md` §3 终态设计；工作区已有按已否决"npm 依赖 + 提取脚本 + 门禁"形态完成的未提交实现，按 revision §6 返工处置——五个 css 产物字节精确直接保留）：1) 机制删除与依赖回退：删 `projects/game/web/frontend/scripts/`（sync-dsh-theme.mjs）；`package.json` 移除 `"@deepseek-ai/dsh-client-ui-theme": "catalog:"`；`BUILD.bazel` vitest data 移除 `:node_modules/@deepseek-ai/dsh-client-ui-theme`（保留 `index.html` 条目）；`pnpm-workspace.yaml` catalog 移除该包条目；`bazel run @pnpm -- --dir /mnt/code/dominion install` 回退 lock 后 `bazel run //:gazelle projects/game/web/frontend`（revision §6）；2) vendored 目录（保留五个 css 与 `index.css`）：`src/dsh-theme/README.md` 改写为溯源与人工升级流程（版本 0.1.1-rc.2、tarball 内嵌实态与 region 锚点、npm pack 再获取步骤、MIT 与上游链接，revision §3.2）；3) 引入与激活（已就位，核查即可）：`src/App.tsx` 在 `import './theme.css'` 前引入 `./dsh-theme/index.css`；`index.html` 的 `<body>` 置静态 `data-ds-dark-theme` 布尔属性；`src/theme.css` `:root` 删除 11 个手写 `--dsw-*` 声明、保留 `--app-*` 并新增 `color-scheme: dark`、头注释更新为指向 dsh-theme sheets（revision §3.3–3.4）；4) 测试：`src/dsh-theme.test.ts` 改写为单一 FR-022 消费覆盖断言（primitives `lib/**/*.css` 与 `src/theme.css` 消费的 `--dsw-*` ⊆ vendored sheets 定义、豁免 `--dsw-hovercard-bg`、失败信息指向 dsh-theme README 升级流程，revision §3.6/§4）；`src/components/SessionList.test.tsx` Menu 卡片视觉断言保留（sheets 定义 `--dsw-specific-menu`/`--dsw-alias-border-inverted`/`--dsw-shadow-lv3`、含 dark 激活选择器、`index.html` 含激活属性、theme.css 零 `--dsw-*` 声明）；`projects/game/web/frontend/README.md` "依赖"节与 attribution 改写（去 ui-theme 依赖与脚本生成表述、保留 primitives 已迁 catalog 的修正，revision §3.7）；5) 验证：`bazel build //projects/game/web/frontend:dist` + `bazel test //projects/game/web/frontend/...` 全绿；核查既有组件（Button/Input/StateDot/ReasoningRow/ToolCard 等）视觉零回归（token 值向官方 dark 值靠拢属预期修复非回归）；人工浏览器目验 `···` 菜单完整卡片形态
