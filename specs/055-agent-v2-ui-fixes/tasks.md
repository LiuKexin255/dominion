...
- [ ] T016 大型测试回归：通过 testplan skill 执行 054 既有测试计划 `projects/game/testplan/system_test.yaml`（含 agent_v2 两 suite 拓扑，`guitar run` 完成部署→测试→清理闭环），全部用例通过（宪法 VI gate 5 + spec A5 回归面承诺；本 feature 无新增大型测试面）
