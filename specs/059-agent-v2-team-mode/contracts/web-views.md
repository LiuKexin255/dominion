...
- **物化表单**：player preset 下拉（role=PLAYER 过滤）+ planner preset 下拉（role=PLANNER 过滤），均必选；player/planner model 下拉各一（可选、留空=部署默认；选项来自 `/api/v2/models`，与校验同源）。Apply = `UpdateTeam`。
- **刷新语义提示**：再次 Apply = 刷新（清空短期记忆、终止在途回合），显式提示（对齐现状）。
- **状态呈现**：物化状态、desktop 连接状态（GetTeam 轮询，节奏沿用现状 10s + 关键时机即时刷新）、成员清单（role + preset + model）。

## 2. 视图模型（每个 team 会话）

| 视图 | 数量 | 数据源（回填） | 数据源（实时） |
|---|---|---|---|
| 团队视图 | 1 | `ListTeamMessages` | Send 流事件按 `member` 归并 + 编排自动驱动的成员回合事件 |
| 成员视角视图 | 2（player/planner） | `ListMemberMessages` | 该成员回合事件（其他成员产出在其被驱动消费前不出现） |

- **视图切换**：对话页顶部切换器（团队 | player | planner）；切换为纯前端状态，不重新回填（各视图历史常驻）。
- **实时事件来源**：编排层自动驱动（非用户 Send 触发）的成员回合事件必须推送至前端（复用现有推送通道语义——具体承载在实现期确定，行为要求：自动驱动的回合在团队视图与对应成员视角视图实时可见，US2 场景 2/4 与 US4 场景 4 的断言依据）。

## 3. 团队视图渲染规范（FR-014）

- 全部消息按 `seq` 时间归并：用户消息（member=USER，现有 user 气泡形态）、各成员原生输出（member=PLAYER/PLANNER）。
...
- 该成员视角消息序列：`role=USER` 且 `sender=USER` → 用户消息气泡；`role=AGENT` → 该成员自己的输出（agent 形态，含工具调用卡）；`role=USER` 且 `sender=PLAYER/PLANNER` → 标注来源的用户消息（渲染为 `user: [sender] 正文`——sender 为另一成员时的广播注入原文）。
