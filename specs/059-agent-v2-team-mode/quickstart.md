...
- 大型测试：`guitar run projects/game/testplan/system_test.yaml`（部署→测试→清理闭环；全量通过 = 验收）。
...
1. 经 `POST /api/v2/templates/saolei/presets` 创建 player/planner preset 各一（role 必填——字符串场景词汇 "player"/"planner"）；`GET .../presets?role=player` 过滤正确；`PATCH` 仅可改 persona，改 role 被拒。
...
### V3 — team 物化与用户首驱（US2 场景 1/2）

1. 未物化 session 直接 `POST ...:send` → `FAILED_PRECONDITION`，web 呈现引导态。
2. `PATCH /api/v2/{team}` 提交 members 配置（player/planner 成员各含 preset + 可选 model，role 为字符串场景词汇）→ 物化成功返回 Team（members/desktop_connected）。
3. **物化后无用户 Send**：team 静止等待——无成员被驱动、无新消息出现（不出现任何编排层合成的驱动消息）。
4. 用户 Send 第一条消息 → planner 被驱动处理并产出开局策略消息（团队视图实时），随后 player 被驱动开始游戏（saolei 工具调用流 + fake-desktop 收到操作）——同一 Send 建立的 team 流持续覆盖上述全部回合（team-api.md §3.1，全程断言见 V4-4）。
...
3. planner 回合结束后**无用户触发**，player 被续驱进入下一轮（结构性续驱——驱动输入 = drain 返回的未消费团队消息（planner 产出广播），无编排层合成消息；夹具可控制"继续开局"与"不开局"两种行为各验证一次）。
4. 全程同一 Send 流（首条用户消息建立）持续覆盖双成员全部回合直至 team 静止：成员事件帧 member 标注正确；`team_message` 帧的 seq 与 List 面回填（V5-1）同源一致；"不开局"情形下流在 player 停止后自然结束（静止收敛，无需用户干预）。

### V5 — 双视图与 system prompt（US4/US5，SC-003/SC-005）

1. `GET /api/v2/.../team/messages`：归并序列含 "user"/"player"/"planner" 标注（字符串值），成员消息为原生输出（含工具调用块），无广播包装形态。
...
1. player 回合中 Send → `queued` 帧；回合结束后由 player 消化。并发流：两流并存期间各自完整接收后续成员事件帧与 `team_message` 帧（重复帧经 seq/(member, turn_id) 锚去重，team-api.md §3.4）。
2. planner 复盘回合结束时存在排队消息 → planner 先消化排队消息、随后才切换续驱 player（夹具构造该时序）。
3. `POST .../team:cancel` → 在途回合 `turn_end{CANCELED}`、续驱暂停（无新回合出现）、活跃 team 流在 Cancel 后的静止点结束；再次 Send → 循环恢复（Send 即有输入即驱动，消息由当前激活成员处理，建立新 team 流）；重复 cancel 幂等成功。
