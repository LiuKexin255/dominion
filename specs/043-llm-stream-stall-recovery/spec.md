...
- Q: Does LangGraph's `idleTimeout` alone cover tool-execution phases (FR-003)? → A: **No — not during the tool's execution.** The idle timer refreshes on callback events (tool start/end) at the tool's **boundaries**, but a long-running OperationBridge dispatch (up to 20 min) emits no events between `handleToolStart` and `handleToolEnd`, and the watchdog is a wall-clock timer (verified in installed `dist/pregel/timeout.js`). A bare `idleTimeout` would therefore raise a false `NodeTimeoutError` mid-tool. The plan adds a **dispatch heartbeat** (research.md R7): `OperationBridge.dispatch()` calls `config.heartbeat()` every `TOOL_HEARTBEAT_INTERVAL_MS` while pending, which refreshes the idle timer unconditionally. This satisfies FR-003's observable requirement (no false stall during tool execution) without custom pause/resume logic.
...
The agent's saolei tools involve desktop operations (mouse clicks, keyboard input) dispatched via the OperationBridge, which legitimately take time — up to 20 minutes — as they wait for the desktop user to respond. The stall detection MUST NOT fire during tool execution: the idle timer is paused when a tool starts executing and resumed when the tool finishes, so a long-running tool is never mistaken for a stalled LLM stream.
...
**Independent Test**: Start an agent turn that invokes a saolei tool (e.g., a mouse click that waits for the desktop). Verify that the tool execution time (which may exceed the idle timeout) does NOT trigger a stall detection — the turn continues normally after the tool returns.

**Acceptance Scenarios**:

1. **Given** an agent turn in which a tool has been invoked and is executing, **When** the tool execution takes longer than the idle timeout, **Then** no stall is detected — the idle timer is paused for the duration of tool execution.
...
- **Stall immediately after a tool finishes**: the idle timer resumes when the tool completes; if the model then stalls, the full idle window must elapse before detection — the prior tool execution does not "credit" a shorter detection window.
...
- **FR-003**: The stall detection MUST distinguish between model-streaming phases (where the idle timer is active) and tool-execution phases (where the timer is paused). The timer MUST be paused when a tool begins executing and resumed when the tool completes (success or error), so that legitimate tool wait time is never mistaken for a model stall.
...
- **SC-003**: In 100% of tool-execution phases that exceed the idle timeout duration, no false stall is detected — the idle timer is correctly paused for the full duration of tool execution.
...
- The stall detection operates at the team-graph event-stream level (the `streamEvents` output consumed by the turn runner), not at the raw HTTP/SSE level. This is because the agent consumes LLM output through the LangGraph event stream, and tool-execution events flow through the same stream — the watchdog must distinguish model-streaming from tool-execution phases (FR-003; implemented as boundary refresh + dispatch heartbeat per research.md R7). The LangGraph `streamEvents` signal-propagation mechanism (already used by the existing abort path, [Feature 026](../026-agent-abort-crash-fix/spec.md)) is relied upon to abort the in-flight model call when the watchdog fires.
...
- Tool execution can legitimately take up to 20 minutes (the OperationBridge dispatch timeout, `projects/game/agent/src/operation-bridge.ts:55`). FR-003's requirement — no false stall during tool execution — is satisfied by the idle-timeout boundary refresh (tool start/end events) plus the dispatch heartbeat (research.md R7), never by assuming tool events stream continuously.
...
- `projects/game/agent/src/operation-bridge.ts:55` — the 20-minute OperationBridge dispatch timeout (`DISPATCH_TIMEOUT_MS = 1_200_000`), the basis for the tool-execution no-false-stall requirement (FR-003). Also the heartbeat call site (research.md R7).
