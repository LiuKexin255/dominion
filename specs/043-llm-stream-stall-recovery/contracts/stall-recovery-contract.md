...
- `refreshOn: "auto"`: the idle timer refreshes on LangChain callback events (model streaming tokens, tool start/end), state writes, and child-task scheduling. This is the only supported mode. **Tool-execution coverage additionally requires the dispatch heartbeat (research.md R7, §1.2 below)** — tool start/end events alone only refresh at tool boundaries, not during a long tool wait.
...
| Explicit `config.heartbeat()` | **NEW: OperationBridge dispatch interval (research.md R7)** | — | ✅ Refreshes unconditionally (not gated on `refreshOn`) — covers the mid-tool gap |

**Invariant**: During normal agent operation, the idle timer is continuously refreshed: model streaming via token deltas, tool execution via the dispatch heartbeat (`config.heartbeat()` every `TOOL_HEARTBEAT_INTERVAL_MS`, default 10s < 30s idle). A stall (model hangs, no tokens, no tool events, no heartbeat) is the ONLY scenario where the timer elapses.

**⚠️ Without the dispatch heartbeat, the idle timer fires DURING any tool execution longer than `idleTimeout`** (the watchdog is a wall-clock `Promise.race`; no LangChain events fire between `handleToolStart` and `handleToolEnd` — verified in installed `dist/pregel/timeout.js`). The heartbeat is therefore a REQUIRED part of FR-003, not an optimization.

**Contract for the heartbeat**:
- `OperationBridge.dispatch(part, signal?, heartbeat?)` — optional `heartbeat: () => void`; called every `TOOL_HEARTBEAT_INTERVAL_MS` (default 10_000, MUST be < `STREAM_IDLE_TIMEOUT_MS`) while the dispatch is pending; interval cleared on resolve/abort/timeout.
- The saolei tools (`mouse_click`, `mouse_move`) pass `config.heartbeat` through to `dispatch()` (the tool invoke config carries it — LangGraph's `wrapConfig` installs `heartbeat` on the node-attempt config and `ToolNode.runTool` spreads `...config` into the tool config).
- If no heartbeat function is provided (non-saolei tools, tests), dispatch degrades to today's behavior (no heartbeat; a long wait with no events would trip the idle timer — acceptable only where the tool is known short).
