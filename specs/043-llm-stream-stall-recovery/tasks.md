...
**Goal**: Verify that the `idleTimeout` with `refreshOn: "auto"` does NOT fire during legitimate tool execution (which can take up to 20 minutes via OperationBridge). Tool start/end events refresh the timer at tool **boundaries**, but during the dispatch await no LangChain events fire — so a **dispatch heartbeat is REQUIRED** (research.md R7): `OperationBridge.dispatch()` calls `config.heartbeat()` every `TOOL_HEARTBEAT_INTERVAL_MS` while pending, keeping the idle timer alive for the full tool wait. This phase implements the heartbeat (production code) and adds the behavioral test.

**Independent Test**: Verify the player/planner node specs include `refreshOn: "auto"` and `idleTimeout` is set (and that `setNodeDefaults` was NOT used). Verify the heartbeat: dispatch a fake tool invoke that hangs > idleTimeout with a heartbeat-providing config → no `NodeTimeoutError`; without heartbeat → `NodeTimeoutError` (documents why the heartbeat exists). For a full tool-execution-timeout test, see the large test in Phase 7. See [quickstart.md](quickstart.md) Scenario 6.

### 文档清单

- **代码规范文档**: `style/javascript.md` 的测试章节（`vitest_test` 宏声明规则 lines 22–52、Mock 约定 lines 54–86；javascript.md 引用的 `specs/019-js-test-reliability/` 与 [vitest Mocking Modules — Pitfalls](https://vitest.dev/guide/mocking/modules#mocking-modules-pitfalls) 一并阅读）；本 phase 涉及生产代码修改（operation-bridge.ts），需阅读 Google TS Style Guide
- **官方文档**: [LangGraph `LangGraphRunnableConfig.heartbeat`](https://github.com/langchain-ai/langgraph/blob/main/libs/langgraph/src/pregel/utils/timeout.ts) — the `wrapConfig` heartbeat function (installed `dist/pregel/timeout.js`, unconditional `scope.touch()` refresh, research.md R7)
- **技术文章**: 无

### Tasks

- [ ] T008 [US3] Add unit test to `projects/game/agent/src/team/graph.test.ts`: build the team graph and verify the player and planner node specs include `timeout.idleTimeout` equal to `STREAM_IDLE_TIMEOUT_MS` and `timeout.refreshOn === "auto"`. This is a configuration verification test — it asserts the timeout is applied to player/planner (per contract §1.1) and that `setNodeDefaults` was NOT used (compress/initInstruction/postCompactInstruction must NOT carry the timeout). A full behavioral test (tool execution > idleTimeout without false positive) is covered by T008b + the large test in Phase 7 (T011).

- [ ] T008b [P] [US3] Implement the dispatch heartbeat in `projects/game/agent/src/operation-bridge.ts`: add an optional `heartbeat?: () => void` parameter to `dispatch(part, signal?, heartbeat?)`; while the dispatch is pending, call `heartbeat()` every `TOOL_HEARTBEAT_INTERVAL_MS` (imported from `../llm`); clear the interval on resolve/abort/timeout (no leaked timers). Then update the saolei tools (`tools/mouse_click/mouse-click.ts`, `tools/mouse_move/mouse-move.ts`) to pass `config.heartbeat` through to `dispatch()` (the tool invoke config carries it — LangGraph's `wrapConfig` installs it on the node-attempt config, verified in `dist/pregel/timeout.js` and `dist/agents/nodes/ToolNode.js`). See contract §1.2 / research.md R7.

- [ ] T008c [US3] Add unit test to `projects/game/agent/src/operation-bridge.test.ts`: (1) with a heartbeat-providing config, a dispatch that resolves after > `STREAM_IDLE_TIMEOUT_MS` (fake timers) invokes `heartbeat` at `TOOL_HEARTBEAT_INTERVAL_MS` cadence and never triggers a `NodeTimeoutError`; (2) a pending dispatch without heartbeat (non-saolei path) documents the degraded behavior; (3) the interval is cleared when the dispatch resolves/aborts (assert no timer leaks via `vi.getTimerCount()`).

**Checkpoint**: US3 complete. The timeout configuration is verified on player/planner only, and the dispatch heartbeat keeps the idle timer alive during long tool execution.
...
- **Phase 2 (Foundational)**: Depends on T001 (constants). BLOCKS US1/US2/US3.
- **Phase 3 (US1)**: Depends on Phase 2 (timeout must be configured) + T001. T003/T004 are parallel (different files). T005/T006 depend on T003/T004 respectively.
- **Phase 4 (US2)**: Depends on Phase 3 (US1 — stall must propagate to finishError for the test to verify buffer retention). No production code change — test only.
- **Phase 5 (US3)**: Depends on Phase 2 (timeout config must be in place) + T001 (TOOL_HEARTBEAT_INTERVAL_MS). T008 (config test) is parallel with T008b (heartbeat implementation); T008c depends on T008b. Production code change: operation-bridge.ts + tools (dispatch heartbeat).
- **Phase 6 (US4)**: Depends on T001 (constants). Independent of Phases 2–5. Can run in parallel.
- **Phase 7 (Polish)**: Depends on all prior phases. T013 (SC-006 regression) depends on Phase 3 (abort path is baseline behavior; test asserts unchanged semantics).

### User Story Dependencies

- **US1 (P1)**: Depends on Phase 2 (node timeout). MVP core.
- **US2 (P1)**: Emergent from US1 (finishError retains buffer). Test verifies the property.
- **US3 (P1)**: Depends on Phase 2 (node timeout) + the dispatch heartbeat (T008b) — the timer is only refreshed at tool boundaries without it, so the heartbeat is REQUIRED (research.md R7). Test verifies config + heartbeat behavior.
- **US4 (P2)**: Independent of US1–US3. Can be developed in parallel.

### Parallel Opportunities

- T003 (player.ts) and T004 (planner.ts) are different files → parallel.
- Phase 5 (US3) and Phase 6 (US4) are independent of Phase 3 → parallel.
- T005 and T006 are different test files → parallel (after their implementation tasks).
- T008 (graph config test) and T008b (heartbeat implementation) are different files → parallel.
...
2. Complete Phase 2: Foundational (node-level idleTimeout on player/planner)
3. Complete Phase 3: US1 (player + planner error classification)
4. **STOP and VALIDATE**: Run T005/T006 unit tests. Verify stall detection works.
5. Complete Phase 4: US2 (buffer retention test)
6. Complete Phase 5: US3 (config test + dispatch heartbeat + behavioral test)
7. **STOP and VALIDATE**: Run all unit tests. The MVP is complete.
...
3. Add US2 + US3 → verify buffer retention + tool-awareness (dispatch heartbeat)
4. Add US4 → init turn timeout
5. Large test → end-to-end validation

---

## Notes

- US2 requires NO production code changes — it is an emergent property of the existing `finishError` terminal, verified by tests. US3 requires the dispatch heartbeat (T008b): LangGraph's `idleTimeout` refreshes on callback events at tool boundaries only; during a long OperationBridge dispatch (up to 20 min) no events fire, so the heartbeat (`config.heartbeat()` every `TOOL_HEARTBEAT_INTERVAL_MS`) is REQUIRED to keep the idle timer alive (research.md R7). This is by design (Constitution §II: the LangGraph `TimeoutPolicy` + heartbeat + existing `finishError` terminal).
