...
- [X] T006 [P] [US1] Rewrite proxy service → TeamService in `projects/game/proxy/handler/handler.go` + `handler_test.go` + `runtime/agentclient/client.go` + `cmd/main.go` — **先删除后新增**：
  ...
  - **新增** `CreateTeam`（POST `templates/*/sessions/*/team`，**唯一 owner 分配点** `assignOwner`——并发竞态幂等，Phase 5 T017a 补全）；`GetTeam`（GET `templates/*/sessions/*/team`，解析 `gameconst.TeamSessionID`，lookup owner，调下游 `client.GetTeam`）；`Connect`（bidi stream，端点 `templates/{template}/sessions/{session}/connect`，**lookup owner——不再分配 owner（分配迁移至 CreateTeam，T017a）**）；`ListMessages`（GET `.../agents/*/messages`，按 agent 分区，解析 `gameconst.MessageAgentParse`）；`RefreshTeam`（POST `.../team:refresh`）。
  - `agentclient/client.go`：`Client` interface 改为 `CreateTeam`/`GetTeam`/`Connect`(TeamService_ConnectClient)/`ListMessages`/`RefreshTeam`；`AgentClient` wrapper 改为 `game.TeamServiceClient`。
...
- [X] T017 [US2] Create `projects/game/agent/src/session-team.ts` — `SessionTeam`（取代 `SessionAgent`）：持有 team graph 实例 + per-session ephemeral buffer + `StrategyStore` 引用 + `OperationBridge`（player 独占）。`SessionTeamStore`（取代 `SessionAgentStore`）：**仅 `create(profile)+get`**（CreateTeam 唯一创建点；map 记录每 session 创建时所用 profile 供幂等比较——重复 create profile 相同幂等、不同 ALREADY_EXISTS；不再有 `getOrCreate` 懒加载，见 T017a）。turn 提交 = 一次 graph invoke（`gameEnded` 在 turn 内由条件边处理，不破坏 TurnLoop 单飞）。复用 TurnLoop 单飞/队列语义（D10）。参照 `contracts/team-graph-contract.md` §6。
- [X] T017a [US2] CreateTeam RPC 垂直落地（proto+proxy+agent，设计决策见 `plan.md`「Team 显式创建」节 + `contracts/api-contract.md` §2.2）：
  - `projects/game/game.proto`：TeamService 新增 `rpc CreateTeam(CreateTeamRequest) returns (Team)`（AIP-133，POST `/api/v1/{parent=templates/*/sessions/*}/team`）；`CreateTeamRequest { parent=Session 资源名; profile=TeamProfile 资源名 }`（template 段 MUST 与 parent 一致，handler 校验）。
  - `projects/game/proxy/handler/handler.go`：`CreateTeam` = **唯一 owner 分配点**（`assignOwner`，`ErrOwnerAlreadyExists` 并发竞态下重读既有 owner）；`Connect` 改为 `lookupOwner`（不再分配 owner）；未创建 → NOT_FOUND。
  - `projects/game/agent/src/handler.ts` + `session-team.ts`：`SessionTeamStore` 仅 `create(profile)+get`（移除原 `getOrCreate` 懒加载）；GetTeam/Connect/ListMessages/RefreshTeam 未创建 → NOT_FOUND；重复 CreateTeam：profile 相同 → 幂等返回既有 Team，profile 不同 → ALREADY_EXISTS（details 含既有 profile）。
  - **移除 `DEFAULT_TEAM_PROFILE`**：profile 由 CreateTeamRequest 显式传入（team 构建于该 profile）。
