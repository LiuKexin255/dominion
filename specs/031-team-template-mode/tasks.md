...
- [X] T006 [P] [US1] Rewrite proxy service → TeamService in `projects/game/proxy/handler/handler.go` + `handler_test.go` + `runtime/agentclient/client.go` + `cmd/main.go` — **先删除后新增**：
  ...
  - **新增** `UpdateTeam`（PATCH `templates/*/sessions/*/team`，**唯一 owner 分配点** `assignOwner`——并发竞态幂等，Phase 5 T017a 补全；040 修订：原 CreateTeam 被 [`specs/040-team-singleton-conformance/`](../../040-team-singleton-conformance/) supersede）；`GetTeam`（GET `templates/*/sessions/*/team`，解析 `gameconst.TeamSessionID`，lookup owner，调下游 `client.GetTeam`）；`Connect`（bidi stream，端点 `templates/{template}/sessions/{session}/connect`，**lookup owner——不再分配 owner（分配迁移至 UpdateTeam，T017a）**）；`ListMessages`（GET `.../agents/*/messages`，按 agent 分区，解析 `gameconst.MessageAgentParse`）；`RefreshTeam`（POST `.../team:refresh`）。
  - `agentclient/client.go`：`Client` interface 改为 `UpdateTeam`/`GetTeam`/`Connect`(TeamService_ConnectClient)/`ListMessages`/`RefreshTeam`；`AgentClient` wrapper 改为 `game.TeamServiceClient`。
...
- [X] T012 [US2] Create `projects/game/agent/src/team/update-strategy.ts` — `update_strategy` tool（仅 planner 持有，写 `StrategyStore.put(sessionId, content)`；schema `{ content: z.string() }`）。参照 `contracts/team-graph-contract.md` §2.4。
- [X] T013 [US2] Create `projects/game/agent/src/team/team-sink.ts` — `teamSink` 实现（`SaoleiEventSink` consumer）：`onGameStart` 可选重置 buffer；`onMove` 更新 `buffer.gameState`；`onGameEnd` 写 `buffer.gameEvent = {state, status, endedAt, consumed:false}` + 更新 `buffer.gameState`。ephemeral buffer 类型定义（per-session 进程内普通对象）。参照 `contracts/saolei-sink-contract.md` §4 + `contracts/team-graph-contract.md` §3。
- [X] T014 [US2] Create `projects/game/agent/src/team/player.ts` — player 节点：**`createAgent`（不带自身 checkpointer，内部 agent loop 跑到 LLM 自停）**；工具 = saolei MCP tools（仅 player）；进入时读 `StrategyStore.get(sessionId)` 作为"当前态势"注入 prompt（FR-015；player 无读取工具）；**后处理**（createAgent 返回后执行一次）：读 ephemeral buffer `gameEvent`，若未 consumed 则写 `TeamState.gameEnded = status`（D6 步骤 4）。流式输出 `ContentBlock` → `AgentFrame`（`agent="player"`）。`accepts_user_input=true`。参照 `contracts/team-graph-contract.md` §2.1 + spike `team-graph.ts`。
- [X] T015 [US2] Create `projects/game/agent/src/team/planner.ts` — planner 节点：system = [复盘指令] + [当前策略 `StrategyStore.get`（初始 `""`，FR-014）]；复盘输入 = ephemeral buffer `gameState`；工具 = 仅 `update_strategy`（FR-012）；`update_strategy` 重试在节点内部处理（D6/需求方 #6）；节点返回后 graph 清 `gameEnded=null`（无条件）。`accepts_user_input=false`。参照 `contracts/team-graph-contract.md` §2.2。
- [X] T016 [US2] Create `projects/game/agent/src/team/graph.ts` — `TeamGraph` builder：`StateGraph(TeamState)` → addNode("player", playerNode) / addNode("planner", plannerNode) → addEdge(START, "player") → addConditionalEdges("player", route(读 `state.gameEnded`): gameEnded≠null→"planner"；null→END) → addEdge("planner", 清 gameEnded=null, "player") → compile({ checkpointer: new MemorySaver() })。模板定义导出 `TeamAgent[]` 描述（`[{name:"player",accepts_user_input:true},{name:"planner",accepts_user_input:false}]`）。参照 `contracts/team-graph-contract.md` §2.3 + spike `team-graph.ts`。**测试断言坑**（D14 注意事项 3）：断言"局结束"检查 `plannerMessages` 非空而非终态 `gameEnded`。
- [X] T017 [US2] Create `projects/game/agent/src/session-team.ts` — `SessionTeam`（取代 `SessionAgent`）：持有 team graph 实例 + per-session ephemeral buffer + `StrategyStore` 引用 + `OperationBridge`（player 独占）。`SessionTeamStore`（取代 `SessionAgentStore`）：**仅 `update(sessionId, template, profileName, allowMissing)+get`**（UpdateTeam 唯一物化点——040 修订：原 `create(profile)+get` 被 [`specs/040-team-singleton-conformance/`](../../040-team-singleton-conformance/) supersede；map 记录每 session 当前所用 profile 供幂等比较；不再有 `getOrCreate` 懒加载，见 T017a）。turn 提交 = 一次 graph invoke（`gameEnded` 在 turn 内由条件边处理，不破坏 TurnLoop 单飞）。复用 TurnLoop 单飞/队列语义（D10）。参照 `contracts/team-graph-contract.md` §6。
- [X] T017a [US2] Team 物化 RPC 垂直落地（proto+proxy+agent，设计决策见 `plan.md`「Team 单例物化」节 + `contracts/api-contract.md` §2.2）**（040 修订：本 task 原实现为 CreateTeam（AIP-133 显式创建），已被 [`specs/040-team-singleton-conformance/`](../../040-team-singleton-conformance/) supersede——移除 CreateTeam、改为 `UpdateTeam(allow_missing=true)` 物化，最终契约见 040 [`contracts/api-contract.md`](../../040-team-singleton-conformance/contracts/api-contract.md) §2）**：
  - `projects/game/game.proto`：TeamService 新增 `rpc UpdateTeam(UpdateTeamRequest) returns (Team)`（[AIP-134](https://google.aip.dev/134) create-or-update + [AIP-156](https://google.aip.dev/156)，PATCH `/api/v1/{team.name=templates/*/sessions/*/team}`）；`UpdateTeamRequest { team=Team 资源（name+profile）; update_mask; allow_missing }`（template 段 MUST 与 `team.name` 一致，handler 校验）；`message Team` 新增 `profile` 字段。
  - `projects/game/proxy/handler/handler.go`：`UpdateTeam` = **唯一 owner 分配点**（`assignOwner`，`ErrOwnerAlreadyExists` 并发竞态下重读既有 owner）；proxy 为路由层始终 `assignOwner`、不 inspect `allow_missing`；`Connect` 改为 `lookupOwner`（不再分配 owner）；未物化 → NOT_FOUND。
  - `projects/game/agent/src/handler.ts` + `session-team.ts`：`SessionTeamStore` 仅 `update(sessionId, template, profileName, allowMissing)+get`（移除原 `getOrCreate` 懒加载）；GetTeam/Connect/ListMessages/RefreshTeam 未物化 → NOT_FOUND；`allow_missing=true` 缺失 → 物化、`allow_missing=false` 缺失 → NOT_FOUND、同 profile → 幂等返回既有 Team、异 profile → 重建 graph（040 FR-001/FR-002/FR-005）；**删除 `TeamAlreadyExistsError`/ALREADY_EXISTS 路径**（040 FR-007）。
  - **移除 `DEFAULT_TEAM_PROFILE`**：profile 由 UpdateTeamRequest 的 team 体显式传入（team 构建于该 profile）。
- [X] T018 [US2] Rewrite `projects/game/agent/src/context-middleware.ts` — RefreshTeam 清空短期记忆：**外层 `graph.updateState` 直清**——`refreshTeamChannels(graph, sessionId)` 一次 `updateState({configurable:{thread_id}}, clearChannel("playerMessages") + clearChannel("plannerMessages"))`，各通道一个 `RemoveMessage({id: REMOVE_ALL_MESSAGES})`（per-channel 独立，spike D14 A1 实测确认）。**偏离注（用户已接受）**：原措辞"`beforeModel` 钩子返回 `{ messages: [RemoveMessage(REMOVE_ALL_MESSAGES)] }`"不可行——player/planner 的 createAgent 不带自身 checkpointer（D14 A2），其 middleware 只见 createAgent 自身 `{messages}` 通道，无法触达外层 `playerMessages`/`plannerMessages`（两通道持久化在外层单一 MemorySaver，D14 A3）；`graph.updateState` 经 channel reducer 应用更新，是架构上正确的落地（详见 `contracts/team-graph-contract.md` §5 偏离说明）。策略（StrategyStore）与 gameEnded 不在清空范围（FR-018）。参照 `contracts/team-graph-contract.md` §5 + spike D14 A4。
- [X] T019 [US2] Rewrite `projects/game/agent/src/prompt-client.ts` — **先删除后新增**：删除 `getProfile(profileName)`（GetAgentProfile）；新增 `getTeamProfile(template, profileName)` → 调 `GetTeamProfile` RPC，返回 `{ playerModel, plannerModel }`（从 `oneof spec.saolei` 解析）。PROMPT_SERVICE_TARGET 不变。参照 `contracts/api-contract.md` §2.3 + `contracts/strategy-store-contract.md`（prompt 服务不参与 strategy）。更新 `prompt-client.test.ts`。
- [X] T020 [US2] Rewrite `projects/game/agent/src/handler.ts` — **先删除后新增**：删除 `GetAgent`/`RefreshAgent` handler 实现；新增 `UpdateTeam`（AIP-134/156——唯一 Team 物化点，未物化 → NOT_FOUND，见 T017a；040 修订：原 CreateTeam 被 supersede）、`GetTeam`（返回 Team + agents 描述，D3；响应含 profile，040 FR-004）；`Connect`（bidi stream，用户输入帧路由给"接受用户输入"的 agent FR-032；frame `AgentFrame.agent` 标识来源 agent）；`ListMessages`（按 agent 分区，从 checkpoint state 按 agent 通道重建历史）；`RefreshTeam`（触发 context-middleware 清空 + invalidate team graph state）。参照 `contracts/api-contract.md` §2.2 + `contracts/team-graph-contract.md` §6。更新 `handler.test.ts`。
- [X] T021 [US2] Rewrite `projects/game/agent/src/server.ts` — **先删除后新增** wiring：删除 SessionAgentStore + AdapterFactory path；新增 SessionTeamStore + StrategyStore(mongo client) + team graph builder + MemorySaver。mongo 客户端初始化（连接当前 mongo 实例，`strategies` 集合）。MCP host lookup 注入 team sink（T009 扩展点）。`model-provider.ts` 适配 TeamProfile（player/planner 各自 ChatModel）。参照 `contracts/team-graph-contract.md` §6 + D10/D11。
- [X] T022 [US2] Delete old single-agent code: `projects/game/agent/src/session-agent.ts` + `session-agent.test.ts`（全删，由 session-team.ts 取代）；rewrite `projects/game/agent/src/llm.ts` — **删除** `AdapterFactory`/`AgentAdapter`/`AgentAdapterImpl`（单 agent adapter 路径），**保留或迁移**仍被 team graph/handler 引用的共享类型（`ContentBlock`/`TurnContent`/`TurnContentPart`/`buildTools`/`MOUSE_TOOL_NAMES`，按实际引用情况决定迁移到 team/ 或保留）；更新 `llm.test.ts`（删除 adapter 测试，保留共享类型测试）。`profile-guard.ts` 适配 team 上下文（或删除如不再需要）。验证 `bazel build projects/game/agent:lib` + `bazel test projects/game/agent:lib_test`。
...
- Phase 5: T010/T011 **可并行**（strategy-store 与 team/state.ts 独立）；T012-T016 sequential（依赖链：update-strategy→team-sink→player→planner→graph）；T017→T017a→T018-T022 sequential（session-team→Team 物化垂直落地→middleware→prompt-client→handler→server→delete-old）
...
5. **外层 `graph.updateState` 直清**：RefreshTeam 落地于 `context-middleware.ts` 的 `refreshTeamChannels`——外层 `graph.updateState` 对两通道各发 `RemoveMessage(REMOVE_ALL_MESSAGES)`（原措辞"`beforeModel` 钩子"不可行：createAgent 无自身 checkpointer，middleware 无法触达外层通道；见 `contracts/team-graph-contract.md` §5 偏离说明）
