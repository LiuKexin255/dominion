...
- [X] T006 [P] 在 `projects/game/pkg/gameconst/const.go` 增加 `AgentV2Target = "game/agent_v2:grpc"`（含注释：agent_v2 服务发现目标，spec FR-013；终态值 `game/agent-v2:grpc`——T045 返工，2026-08-29 第三批裁定，`specs/049-agent-v2-dsh-init/research.md` D13）；`bazel test //projects/game/pkg/gameconst/...`
...
- [X] T040 [P] **（返工：插件空 key 跳过 Authorization——2026-08-29 用户指令第二批，`specs/049-agent-v2-dsh-init/research.md` D9 / `specs/049-agent-v2-dsh-init/contracts/glm-llm-plugin.md` §3 义务 6）** `common/js/dsh-plugins/llm-glm/src/adapter.ts` `stream()`：移除"空 key 即 `assertUsableApiKey` throw `INVALID_CREDENTIAL`"语义——`env(apiKeyEnv)` 为空/未设（trim 空）→ 请求头**不含** Authorization、照常发出请求；非空 → 经 `assertUsableApiKey` 校验（保留 header 安全字符诊断）后携带 `Authorization: Bearer <key>`；attribution headers 任何情形必带；`adapter.test.ts` 同步："throws when the API key env is empty" 用例替换为"空 key → 请求头无 Authorization 且流照常消费"断言（`specs/049-agent-v2-dsh-init/contracts/glm-llm-plugin.md` §6 测试义务 4），既有非空 key Bearer 断言保留；`bazel test //common/js/dsh-plugins/llm-glm:lib_test`
...
- **技术文章/技术参考文档**：`specs/049-agent-v2-dsh-init/contracts/conversation-api.md`（§1 托管拓扑/事件映射 §4/错误映射与两跳语义 §2/§2.1）；`specs/049-agent-v2-dsh-init/contracts/web-frontend.md`；`specs/049-agent-v2-dsh-init/data-model.md`（§2.2 owner 亲和/§2.6–2.9 实体与路由映射）；`specs/049-agent-v2-dsh-init/research.md`（D2/D3/D4/D9/D10/D11/D12/D13——D13 为第三批返工 T044–T046 依据）；`specs/047-dsh-chat-demo/research.md`（D2 resolve 时序/D3 idle 终止/D5 get-or-create）；`specs/002-deploy-secret-config/contracts/secret-config.md`（service/deploy secret 契约）；`tools/release/deploy/README.md`（`runtime_protos` 按标准导入路径物化语义，T016/T017；§服务类型 `artifact.name` 按名选用，T042/T044）；第三批返工机制依据（T044–T046）：`projects/infra/deploy/domain/service_endpoints_name.go`（服务名 pattern 约束）、`common/gopkg/solver/deploy_http_client.go` 与 `common/gopkg/solver/deploy_stateful_resolver.go`（`ErrServiceNotFound` sentinel 与 `%w` 包装链、零实例容忍先例）、`common/gopkg/bootstrap/daemon.go`（Daemon 重启/致命语义）；样板源码：`experimental/dsh/demo/agent/src/{bootstrap,dsh,session,server}.ts`、`experimental/dsh/demo/agent/cordis.yml`、`experimental/dsh/demo/agent/BUILD.bazel`、`experimental/dsh/demo/testplan/deploy.yaml`（env 注入样例）、`projects/game/proxy/cmd/main.go`（路由设施装配样板）、`projects/game/proxy/handler/handler.go`（owner 解析/assignOwner/propagateAgentError 样板）、`projects/game/proxy/domain/interfaces.go`（OwnerStore/OwnerPicker 契约）、`projects/game/proxy/runtime/agentclient/{manager,client}.go`（stateful 实例连接管理）、`projects/game/proxy/runtime/mongo/owner_store.go`（owner 存储实现）、`projects/game/proxy/runtime/picker/hash_picker.go`、`projects/game/pkg/bind/{binder,first_frame}.go`（v1 泵语义，扩展基线）、`projects/game/BUILD.bazel`（根 proto 目标样板）、`projects/game/agent/BUILD.bazel`（ts_proto_library 跨目录引用先例）、`projects/game/gateway/cmd/main.go`、`experimental/js/vite_react_demo/server/main.go` 与 `main_test.go`（Go 静态托管逐行对照，T022）、`projects/game/game.proto`（Session 字段对照，T019）

- [X] T013 [US1] 实现 `projects/game/agent_v2/src/dsh.ts`：`bootDsh()` = boot 前配置注入（`GLM_BASE_URL` 已设直用；否则 `GLM_LLM_TARGET` 经 `createResolver().resolve()` 注入；否则默认 `https://open.bigmodel.cn/api/v1`；`GLM_API_KEY` 三级解析：env 已设直用 → `$DOMINION_SECRET_DIR/glm-api-token` 文件 → 皆无保持未设 + log warning（不含 key 内容）继续 boot，**不 fail-loud**——终态描述，原 fail-loud 实现由 T041 返工收敛（2026-08-29 用户指令第二批，`specs/049-agent-v2-dsh-init/research.md` D9））+ `boot(binName, cordisConfigPath(), undefined, undefined, import.meta.url)`；DI seam（resolver/boot 可注入）+ `dsh.test.ts`（`specs/049-agent-v2-dsh-init/research.md` D9；对照 `experimental/dsh/demo/agent/src/dsh.ts`）
...
- [X] T038 [P] [US1] agent_v2 有状态声明：`projects/game/agent_v2/service.yaml` `kind: stateless` → `kind: stateful`（对齐 v1 agent `projects/game/agent/service.yaml`；proxy 经 `dominion-stateful` 按实例序号寻址，`specs/006-grpc-js-service-discovery` FR-009/FR-012）；`projects/game/pkg/gameconst/const.go` `AgentV2Target` 注释更新（消费方 = proxy 有状态实例解析，gateway 不直连；注释终态随 T045 命名返工同步）
- [X] T041 [US1] **（返工：token 解析容忍缺失——2026-08-29 用户指令第二批，`specs/049-agent-v2-dsh-init/research.md` D9）** `projects/game/agent_v2/src/dsh.ts` `resolveGlmApiKey` → 三级语义：env 已设（trim 非空）直用；否则读 secret 文件（文件缺失/读取失败/内容空白**不再 throw**）；皆无 → `GLM_API_KEY` 保持未设（不写入 env）、经 `@dominion/common-js-logs` 的 `warn` 记录一条（含 env 名 `GLM_API_KEY` 与 secret 文件路径，不含任何 key 内容——SC-004）后继续 boot；boot/resolver 失败的 fail-loud 路径（FR-002 组合启动失败）保持不变；`dsh.test.ts` 同步：删除"文件缺失/文件空 fail-loud"两用例，改为"容忍缺失：warning + boot 继续且 `GLM_API_KEY` 未设"断言，并更新"env 优先"用例内残留的 dummy-key 注释（测试部署不再注入该 env）；`projects/game/agent_v2/cordis.yml` `apiKeyEnv` 行注释更新为三级解析终态；`projects/game/agent_v2/README.md` token 节与运维预置节按 D9 终态改写（生产 SHOULD 预置、测试零 secret）；`bazel test //projects/game/agent_v2:lib_test`
- [X] T042 [P] [US1] **（返工：service.yaml 双 artifact——2026-08-29 用户指令第二批，`specs/049-agent-v2-dsh-init/research.md` D9；artifact 终态名遵循 D13 命名约束）** `projects/game/agent_v2/service.yaml` `artifacts` 增加测试条目 `name: agent-v2-test`（`target: :cmd_image`、`tls: true`、**无 secrets**），生产条目 `agent-v2`（服务名与 artifact 名不含下划线——T044 命名返工并轨，2026-08-29 第三批裁定）；`projects/game/deploy.yaml` 生产绑定（artifact `name: agent-v2` + `glm-api-token: {secret: llm-secrets, key: glm-codingplan}`）**零改动**（`tools/release/deploy/README.md` §服务类型：deploy 经 `artifact.name` 按名选用，编译器 secret 双向校验仅对所选 artifact 生效）；`bazel build //projects/game/agent_v2:cmd_image`
- [X] T044 [P] [US1] **（返工：服务发现名与声明层改名——2026-08-29 第三批裁定，`specs/049-agent-v2-dsh-init/research.md` D13；deploy API 约束 `^[a-z][a-z0-9-]{0,19}$` 禁下划线，`projects/infra/deploy/domain/service_endpoints_name.go:14`，原名 endpoints 查询 400）** `projects/game/agent_v2/service.yaml`：`name: agent_v2` → `name: agent-v2`（k8s 清洗名一致，重部署零 churn）；`projects/game/deploy.yaml` agent_v2 条目 `name: agent_v2` → `name: agent-v2`（secret 绑定不变）+ 条目注释服务名同步；artifact 名由 T042 终态（`agent-v2`/`agent-v2-test`）与 T043（testplan 条目 `name: agent-v2-test`）承载；**目录/文件名/proto/bazel target/Mongo collection（`game_proxy.agent_v2_owners`）一律不动**（区分规则 D13）；`bazel build //projects/game/agent_v2:cmd_image`；验证：`guitar` 部署后 `GET /v1/deploy/scopes/{scope}/environments/{env}/apps/game/services/agent-v2/endpoints` 返回 200（不再 400）
- [X] T045 [P] [US1] **（返工：发现 target 与 OTel 服务身份串——2026-08-29 第三批裁定，`specs/049-agent-v2-dsh-init/research.md` D13）** `projects/game/pkg/gameconst/const.go`：`AgentV2Target = "game/agent_v2:grpc"` → `"game/agent-v2:grpc"`（注释同步：服务发现名 `agent-v2`、项目目录 `agent_v2`；`solver.ParseTarget` 对 service 段无字符校验，`common/gopkg/solver/target.go:100-129`）；`projects/game/agent_v2/src/bootstrap.ts:41` `createOTelReporter("game/agent_v2")` → `("game/agent-v2")`（SigNoz 服务名，与同文件日志字段 `game-agent-v2` 一致）；两处均无其他字面量消费方（proxy `cmd/main.go` 经 `gameconst.AgentV2Target` 引用）；`bazel test //projects/game/pkg/gameconst/... //projects/game/proxy/...`
- [X] T046 [US1] **（返工：proxy 容忍管理服务缺席——2026-08-29 第三批裁定，`specs/049-agent-v2-dsh-init/research.md` D13 / spec FR-016）** `projects/game/proxy/runtime/agentclient/manager.go` `refresh()`：resolver 错误 `errors.Is(err, solver.ErrServiceNotFound)`（HTTP 404 → 裸 sentinel `common/gopkg/solver/deploy_http_client.go:18/56-57`，经 `deploy_stateful_resolver.go:48` 与本函数 `%w` 包装链可达——无需新包装约定）→ warn 日志（app/service 字段）后 `instances = nil` 走既有收尾（陈旧连接关闭、连接表清空、返回 nil），服务出现后周期刷新（30s）自然发现；其余错误类别（400 invalid name、5xx、网络）与 daemon 重启策略**零改动**（400 走普通 error 路径非 sentinel，保持致命语义）；实现形态照 D13 接口签名级代码块；`manager_test.go` 增两用例：(1) resolver 返回包装后的 `ErrServiceNotFound` → refresh 成功且既有 entries 被清空，(2) 后续刷新服务出现 → 连接重建；既有全部用例回归；`bazel test //projects/game/proxy/...`——此修复是 SC-005 必要条件（T037 起 proxy 恒装配 v2 daemon，既有 suite 不含 agent-v2 即触发同一 crash loop）
...
- **官方文档**：无
- **技术文章/技术参考文档**：`.opencode/skills/testplan/SKILL.md`（guitar 操作入口，T033 执行依据）；`specs/049-agent-v2-dsh-init/quickstart.md`（§2 用例集/§3 冒烟/§4 回归）；`specs/049-agent-v2-dsh-init/contracts/fake-responses-wire.md`（§4 部署替换与零 secret）；`specs/049-agent-v2-dsh-init/research.md`（D13 命名区分规则——T047 sweep 依据）；`tools/release/deploy/README.md`（§服务类型 `artifact.name` 按名选用语义与 secret 双向校验，T043）；既有样板：`projects/game/testplan/system_test.yaml`、`projects/game/testplan/deploy_agent.yaml`、`projects/game/testplan/BUILD.bazel`（go_largetest 声明）、`projects/game/testplan/helpers_test.go`

- [X] T030 编写 `projects/game/testplan/deploy_agent_v2.yaml`（type test，services：mongo(dev-single)/session/fake-llm/**proxy**/agent-v2/web/gateway——proxy 为 `/api/v2` 两跳路由（gateway→proxy→agent-v2）的必经组件；agent-v2 选用**测试 artifact `agent-v2-test`**（service.yaml 双 artifact 之无 secret 声明者——`specs/049-agent-v2-dsh-init/research.md` D9；artifact 名遵循 D13 命名约束）→ **零 secret 绑定**；agent-v2 env 仅 `GLM_LLM_TARGET=dominion:///game/fake-llm:8080`（不注入 `GLM_API_KEY`——bootstrap 容忍缺失、插件空 key 免 Authorization，fake header 容忍 `specs/049-agent-v2-dsh-init/contracts/fake-responses-wire.md` §1/§4）；gateway http 块 matches `/api/v1/`+`/api/v2/`，web http 块 hostname `game.liukexin.com` match `/`——对照 `projects/game/testplan/deploy_agent.yaml`）
- [X] T031 编写大型测试用例 `projects/game/testplan/agent_v2_conversation_test.go`（模块=agent_v2 对话面；用例：流式事件序与多帧 delta、多轮连续性、会话隔离、排队 queued→自动续跑（长延迟模板窗口）、history 回填一致性、dispose ABORTED 与全新会话、模型故障 ERROR 恢复、空文本 400；helpers 优先复用/扩展 `helpers_test.go`，trace_id 经 `common/gopkg/otel/tracecontext`——`style/large_test.md` §测试用例）+ `projects/game/testplan/web_test.go`（模块=web 服务：`GET /` HTML、静态资源、页面经 `/api/v1`+`/api/v2` 的管理闭环 smoke）；`projects/game/testplan/BUILD.bazel` 增两个 `go_largetest` target（srcs/deps 对照既有声明，gazelle 同步）
- [X] T032 在既有 `projects/game/testplan/system_test.yaml` 增加 suite（引用 `//projects/game/testplan/deploy_agent_v2.yaml` + T031 两个 case target；**不得新建独立测试计划 YAML**——`style/large_test.md` §测试计划数量）；suite 描述写明覆盖的 spec 场景锚点
- [X] T043 **（返工：测试部署零 secret——2026-08-29 用户指令第二批，`specs/049-agent-v2-dsh-init/research.md` D9 / `specs/049-agent-v2-dsh-init/contracts/fake-responses-wire.md` §4）** 将现有 `projects/game/testplan/deploy_agent_v2.yaml` 收敛至 T030 终态：`artifact.name` 改指 `agent-v2-test`（T042 声明；终态名遵循 D13——T044 命名返工并轨），删除 `secrets` 绑定块与 env `GLM_API_KEY: dummy-key`（保留 `GLM_LLM_TARGET`），删除文件内"secret 绑定强制"相关注释；**移除测试集群 `llm-secrets` 中的占位 `glm-codingplan` key**（T033 首跑期间为使部署通过曾临时预置——按用户指令消除测试对集群 secret 的一切依赖）；依赖 T042（测试 artifact 声明）；`guitar` 部署验证 agent-v2 StatefulSet 就绪不再依赖 secret 卷
- [X] T047 **（返工：仓库内服务名 sweep——2026-08-29 第三批裁定，区分规则与误伤边界 `specs/049-agent-v2-dsh-init/research.md` D13）** 将仓库内（specs/049 文档已由方案修订完成，不在本 task 范围）**服务名指称**改 `agent-v2`：`projects/game/testplan/system_test.yaml`（suite 12 描述：`gateway→proxy→agent_v2` 等拓扑链与服务指称）、`projects/game/testplan/README.md`（指称 suite/服务的行文——suite 名本为 `agent-v2-conversation`）、`projects/game/agent_v2/README.md` 与 `projects/game/web/README.md`（服务说明中的发现名 target/artifact 名）；**保持不动**：文件名/目录/bazel target/collection（`game_proxy.agent_v2_owners`）/proto/包名/env；`rg -n "game/agent_v2|name: agent_v2|artifact `agent_v2`" projects/` 复核零残留（collection `agent_v2_owners` 除外）
- [X] T033 经 testplan skill 实际执行验收：`guitar run projects/game/testplan/system_test.yaml`（新 suite；完成部署→测试→清理闭环），**全部用例通过**（failed/flaky 即修复重跑直至全绿——constitution 原则 VI；禁止以 `bazel build` 测试 target 替代执行）；执行前提：T040–T046 完成（T044–T046 为第三批返工：命名合规 + proxy 容忍——本轮重跑同时证明测试部署零 secret 独立性与 agent-v2-only 拓扑下 proxy 存活）、测试集群无 `glm-codingplan` key（`specs/049-agent-v2-dsh-init/spec.md` FR-008）
- [X] T034 真实 GLM 端点冒烟交付文档：`specs/049-agent-v2-dsh-init/quickstart.md` §3 复核（步骤可手工复验）；`projects/game/agent_v2/README.md` 与 `projects/game/web/README.md`（服务说明、`glm-codingplan` k8s secret key 运维预置说明、已知限制——多标签页无实时推送/desktop 删除不联动/历史随进程重启丢失，`specs/049-agent-v2-dsh-init/research.md` 已知限制节）
- [X] T035 存量零回归与泄漏扫描：`bazel test //projects/game/gateway/... //projects/game/agent/... //projects/game/proxy/... //projects/game/pkg/bind/... //projects/game/fake-llm/... //projects/game/desktop/...` 全绿（SC-005——proxy/bind 为本 feature 增量修改点，其既有 v1 用例必须全绿）；`rg -i "glm-api-token|GLM_API_KEY" --glob '!*.md'` 复核交付物无明文 token 值（SC-004）；既有 game 大型测试抽跑一组（`guitar run projects/game/testplan/system_test.yaml` 既有 suite）确认 `/api/v1` 行为与 proxy v1 TeamService 链路不变
...
- **Phase 2 (Foundational)**: 依赖 Phase 1（T008 需 T003 骨架；T009–T011 需 T002；T012 独立但同期）——**阻塞全部 user story**。T040（插件空 key 返工，2026-08-29 第二批用户指令）独立于既有任务，可即时并行。
- **Phase 3 (US1, MVP)**: 依赖 Phase 2（T013–T015 依赖 T008/T011；T021 依赖 T019/T020；T022 依赖 T004/T005）。**Phase 3 内返工链（2026-08-29 方案变更）**：T039（proto 归位）先行，阻塞 T016/T017/T018/T037；T036（bind 扩展）阻塞 T037（proxy 泵消费方）；T038 独立。**第二批返工（2026-08-29）**：T041（dsh.ts token 容忍）与 T042（service.yaml 双 artifact）互相独立、可与既有任务并行。**第三批返工（2026-08-29 裁定）**：T044（声明层改名）与 T045（target/身份串）互相独立；T046（proxy 容忍）独立；T044 与 T042/T038 同文件（service.yaml）**串行**（建议 T042 → T044 顺序执行，一次编辑收敛双 artifact + 服务名终态）。
...
- **Phase 7 (验收)**: 依赖 Phase 3–6 全部完成 + 返工 T040–T047；**T042（测试 artifact 声明）阻塞 T043（测试部署零 secret）**；T044–T046（第三批返工）与 T047（仓库 sweep，依赖 T044 命名落地）全部完成阻塞 T033（重跑需零 secret 独立性 + 命名合规 + proxy 容忍）。
...
- Phase 2: T008/T009+T010/T012 三线并行；T011 汇合 T009/T010；T040（空 key 返工）独立可并行。
- Phase 3: 返工线 T039 →（T016+T017 → T037 → T018）与 T036（bind 扩展，独立包）、T038（service.yaml 声明）、T041（dsh.ts token 容忍）、T042→T044（service.yaml 双 artifact → 服务名改名，同文件串行）并行；T045（gameconst/bootstrap.ts 身份串）与 T046（proxy 容忍，独立包）可并行；前端线 T019+T020 并行；T021 汇合。
