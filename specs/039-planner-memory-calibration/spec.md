...
**Amended**: 2026-08-08（memory 工具改为 hermes 式单工具 + 参数；planner 注入改为纯内容（不再注入 memory_id）；saolei_operate 改为 hermes 式双形态参数；新增 memory skill。详见 Clarifications Session 2026-08-08。本 spec 仍为 Draft、未实现，故就地修订而非新建 spec。）
...
- **memory 资源 pattern**：`templates/{template}/sessions/{session}/memories/{memory}`（集合段采用复数 `memories`，与本仓库 `messages/{message}`、`profiles/{profile}` 的 AIP-122 约定一致；"memory"作为可数资源条目，复数 "memories"）。template 与 session 经独立 mcp server path 闭包注入（同 saolei mcp 既有做法）。注入系统提示词时 MUST 以 `memory_id: 内容` 形式呈现，使 LLM 能定位要修改的条目；`memory_add`/`memory_update`/`memory_remove` MUST 包含 `memory_id` 参数，其余参数参考 hermes（content 等）。
  > ⚠️ **本条后半段（"memory_id: 内容" 注入格式 + "memory_add/update/remove 含 memory_id 参数"）已被 Session 2026-08-08 推翻**：见下"Session 2026-08-08"的两条 supersede（memory 工具改为 hermes 式单一工具、planner 注入改为纯内容）。资源 pattern `templates/{template}/sessions/{session}/memories/{memory}` 本身不变（`{memory}` 为 memory 服务内部 memory_id）。

### Session 2026-08-08（需求方 directive，记忆工具 / 落子工具 / skill 修订）

> 以下决策在 hermes 记忆工具参数调研（`research.md` D11，来源 [hermes `tools/memory_tool.py`](https://github.com/NousResearch/hermes-agent/blob/main/tools/memory_tool.py) `MEMORY_SCHEMA`）之后作出，**推翻并取代** Session 2026-08-07 中关于记忆工具参数与 planner 注入格式的部分表述（见各条 supersede 标注）。memory 服务的存储 API（`MemoryService` Create/Update/Delete/List，资源 pattern 不变）**保持不变**——变更仅限 MCP 工具面、注入格式与 agent 转换逻辑。

- **（supersede 2026-08-07「memory_add/update/remove 含 memory_id 参数」）memory 工具 → hermes 式单一工具**：planner 持有的记忆工具由三个独立工具（`memory_add`/`memory_update`/`memory_remove`，含 `memory_id`）改为**一个** hermes 风格 `memory` 工具。工具参数与 hermes [`memory` schema](https://github.com/NousResearch/hermes-agent/blob/main/tools/memory_tool.py) 对齐：`action`（`add`/`replace`/`remove`，单次操作）、`content`（条目正文，add/replace 用）、`old_text`（定位既有条目的**短子串**，replace/remove 用）、`operations`（批量原子操作数组，每项 `{action, content?, old_text?}`）。**不设 `target`**（dominion planner 仅有单一记忆存储，无 hermes 的 memory/user 双存储区分）。**不设 `memory_id` 参数**——`memory_id` 退化为 memory 服务内部存储键（资源 id），由 agent 在 `add` 时内部生成（非 LLM 提供），对 LLM 不可见。单次操作（直接 `action`/`content`/`old_text`）与批量（`operations` 数组）二选一，同 hermes。
- **（supersede 2026-08-07「注入系统提示词时 MUST 以 memory_id: 内容 形式呈现」）planner 注入 → 纯内容（hermes 风格）**：注入 planner 系统提示词的冻结记忆快照改为**纯内容**呈现（hermes 风格：条目正文，无 `memory_id` 前缀），使 LLM 据条目内容用 `old_text` 子串定位要 replace/remove 的条目。此与上一条（工具用 `old_text` 子串、无 `memory_id` 参数）**耦合自洽**——定位机制与注入格式一致（详见 `research.md` D11 第三节）。冻结快照语义不变（FR-010，压缩边界刷新）。
- **（转换 + 错误语义）memory 服务 API 不变，agent 转换；0/多命中报错**：`MemoryService` proto（Create/Update/Delete/List，资源 `templates/{template}/sessions/{session}/memories/{memory}`，`{memory}` 即内部 memory_id）**不变**。agent 侧 `memory` 工具实现负责将 hermes 式调用转换为服务调用：`add` → agent 生成 memory_id 后 `CreateMemory`；`replace`/`remove` → agent 经 `ListMemories` 取条目、按 `old_text` **子串匹配**定位唯一条目得其 memory_id → `UpdateMemory`/`DeleteMemory`。**匹配语义同 hermes**：0 命中 → 返回错误文本（含当前条目列表助 LLM 重选）；**多个不同条目**命中同一子串 → 返回错误文本（要求更具体的子串）；多条目全相同 → 作用于首条。`add` 的等价内容已存在 → 视为成功（去重，同 hermes "no duplicate added"）。
- **（augment 2026-08-07「saolei_operate 含全部操作列表」）saolei_operate → hermes 式双形态参数**：`saolei_operate` 参数改为与 hermes `memory` 工具一致的双形态——**普通参数**（单次操作：`type`∈{click/flag/chord}、`x`、`y`）**或** **`operations` 数组**（批量：`[{type, x, y}, ...]`，保留顺序）。单次与批量二选一（同时提供或均不提供由 plan 决定拒绝/优先规则；约束：二者语义等价——单次等价于长度 1 的 operations）。执行与失败细分语义不变（FR-002：无害空操作跳过继续 / 游戏结束 / 结构性错误停止 / 单次返回最终棋盘）。`saolei_init`/`saolei_remain` 不变。gameLog 仍以**一次 `saolei_operate` 调用**（无论单次或批量）为单位记录为一条含其全部操作的历史项。
- **（新增）memory skill（planner 专属）**：参考 hermes 的记忆工具引导（`research.md` D11，HOW/WHEN/SKIP），为 memory mcp 编写配套 `SKILL.md`（`projects/game/agent/src/skill/memory/SKILL.md`，遵循 `specs/020-agent-resources-layout/contracts/skill-md-format.md`），由 planner 系统提示词按既有 skill 机制注入（与 saolei skill 注入 player 对称）。skill 内容适配 dominion 复盘域与冻结快照语义：何时记（player 重复错误模式、被验证有效的策略、复盘认知）、跳过什么（单局偶发失误、具体坐标、易重推事实）、冻结快照模型（改存储不立即生效、压缩边界刷新）、`old_text` 子串定位用法。memory skill 仅注入 planner（player 不持记忆工具，不注入）。
...
player（独占桌面控制的 agent）在观察扫雷棋盘后经常需要一次性连续执行多个落子动作（揭示一片、连续标记、双击展开等）。当前每个落子是独立工具调用（`saolei_click`/`saolei_flag`/`saolei_chord_click`），每次返回一次结果，LLM 批量执行大量操作时产生多次往返。本故事将这三个落子工具合并为单个 `saolei_operate` 工具，其参数采用与 hermes `memory` 工具一致的双形态（Session 2026-08-08）：调用方可传入**普通参数**（单次操作 `type`∈{click/flag/chord} + 坐标 `x`/`y`），**或**传入**`operations` 数组**（批量有序操作列表 `[{type, x, y}, ...]`）。无论单次或批量，操作均按序依次校验并执行，只返回一次结果（反映执行后的最终棋盘与状态）；单次等价于长度 1 的批量。`saolei_init`（开局）与 `saolei_remain`（只读查询）保持独立不变。同步给 planner 的游戏历史也改为以 `saolei_operate` 为单位记录（每次调用——无论单次或批量——作为一条含全部操作的历史项），并使注入 planner 的工具描述说明 `saolei_operate` 及其支持的全部操作类型，与游戏历史一一对应。
...
**Independent Test**: 可在具备桌面（或桩桌面）的环境中独立验证：`saolei_operate` 接收单次操作（普通参数）或有序操作列表（`operations` 数组），操作按顺序依次执行，只返回一次结果；单次参数等价于长度 1 的 `operations`；`saolei_init`/`saolei_remain` 行为不变；同步给 planner 的历史以 `saolei_operate`（含全部操作）为单位记录，且 planner 工具描述包含 `saolei_operate` 与 click/flag/chord 三种操作。
...
2. **Given** 一个已开局的游戏，**When** player 调用 `saolei_operate` 传入一个含多个操作（click/flag/chord 混合）的 `operations` 有序列表，**Then** 操作按列表顺序依次校验并执行，工具只返回一次结果（反映最终棋盘与游戏状态）。
3. **Given** `saolei_operate` 以**普通参数**（单个 `type` + `x` + `y`）调用，**When** 执行，**Then** 其行为等价于以长度 1 的 `operations` 调用（同样校验、同样落子、同样返回棋盘与状态行），亦等价于原对应单次落子工具。
...
7. **Given** 同步给 planner 的游戏历史，**When** player 执行一次 `saolei_operate` 调用（无论单次或批量），**Then** 该次调用在历史中记录为一条含其全部操作的 `saolei_operate` 项（而非拆成多条单步记录）。
...
planner 作为复盘规划者需要跨多局累积对 player 的校准经验与自身认知演化。当前 planner 与 player 共享一个被整体替换的策略文本（`StrategyStore`），LLM 每次从零重写，既贵又易丢失累积洞察。本故事为 planner 建立其**自身**的长期记忆能力：新建一个 memory 服务（grpc-go）承载长期记忆条目，并在 agent 服务上新建 memory mcp server 向 planner 暴露**一个** hermes 风格 `memory` 工具（仅 planner 持有，参数 `action`/`content`/`old_text`/`operations`，详见 Session 2026-08-08）；memory mcp server 由 agent 服务将请求转发到 memory 服务，而非 mcp server 直连 memory 服务，agent 负责将 hermes 式调用转换为 memory 服务的 memory_id 式 RPC。planner 的长期记忆以**冻结快照**方式注入 planner 系统提示词（一次烘焙、跨多次复盘保持冻结，调研 D2），并在压缩边界刷新（调研 D4）；注入为**纯内容**呈现（hermes 风格，无 `memory_id` 前缀），使 LLM 据条目内容用 `old_text` 子串定位要 replace/remove 的条目。配套一个 **memory skill**（planner 专属，参考 hermes 引导）注入 planner 系统提示词，指导何时/如何使用记忆工具与冻结快照模型。memory 资源以会话为作用域：`templates/{template}/sessions/{session}/memories/{memory}`，其中 template 与 session 经独立 mcp server path 闭包注入（同 saolei mcp 既有做法）。
...
**Independent Test**: 可独立验证：memory 服务持久化记忆条目并经 agent 转发可被 `memory` 工具增删改；planner 持有且仅持有这一个记忆工具（hermes 式 `action`/`content`/`old_text`/`operations`，无 `memory_id` 参数）；记忆以冻结快照注入 planner 系统提示词为纯内容（无 `memory_id`）；`replace`/`remove` 经 `old_text` 子串定位，0/多命中报错；memory skill 注入 planner；记忆跨进程重启持久；mcp server 不直连 memory 服务（请求经 agent 转发）；memory 服务存储 API 不变。

**Acceptance Scenarios**:

1. **Given** planner 的工具面，**When** 审查其工具，**Then** planner 持有**一个** `memory` 工具（hermes 风格，含 `action`∈{add/replace/remove}、`content`、`old_text`、`operations` 参数，无 `memory_id` 参数、无 `target` 参数），且无任何策略读取/写入工具（无 `update_strategy`）。
2. **Given** memory mcp server 与 memory 服务，**When** planner 调用 `memory(action=add, content=...)`，**Then** 请求经 agent 服务转发到 memory 服务，agent 内部生成 memory_id 后调 `CreateMemory`，记忆被持久化（条目可经 `memory(action=replace/remove, old_text=...)` 由 agent 子串定位 memory_id 后修改/删除）。
...
4. **Given** `memory` 工具，**When** 审查其参数与转换，**Then** 工具参数为 hermes 式（`action`/`content`/`old_text`/`operations`，无 `memory_id`）；agent 将其转换为 memory 服务的 memory_id 式 RPC（`add`→agent 生成 id+Create；`replace`/`remove`→agent 经 ListMemories + `old_text` 子串匹配定位 id 后 Update/Delete）；**0 命中 → 错误文本（含当前条目助 LLM 重选）；多个不同条目命中 → 错误文本（要求更具体子串）**；memory 服务存储 API 不变。
5. **Given** planner 的系统提示词，**When** 注入长期记忆冻结快照，**Then** 每条记忆以**纯内容**呈现（hermes 风格，无 `memory_id` 前缀），使 LLM 能据内容用 `old_text` 子串调用 replace/remove。
...
9. **Given** planner 的系统提示词，**When** 审查，**Then** 其包含 memory skill 引导（planner 专属，指导何时记/跳过什么/冻结快照模型/`old_text` 用法），且 player 的系统提示词不含该 skill（player 不持记忆工具）。
...
- **saolei_operate 双形态参数的非法组合（Session 2026-08-08）**：调用同时提供普通参数（`type`/`x`/`y`）与 `operations` 数组、或两者均不提供时的行为（拒绝 / 优先取其一）由 plan 决定；本 spec 约束二者语义等价（单次等价长度 1 的 operations）。空 `operations` 列表的行为（视为无操作返回当前状态 / 视为非法）由 plan 决定；本 spec 约束不产生任何落子副作用。
...
- **`old_text` 子串 0/多命中（Session 2026-08-08，取代原"memory_id 冲突"）**：`memory(action=replace/remove)` 的 `old_text` 子串在当前记忆集合中 **0 命中** → 工具返回错误文本（含当前条目列表助 LLM 重选子串）；**多个不同条目**命中同一子串 → 返回错误文本（要求更具体子串）；多条目内容完全相同 → 作用于首条（去重语义）。`memory(action=add)` 的等价内容已存在 → 视为成功（去重，同 hermes "no duplicate added"，非错误）。agent 在转换时保留与服务一致的语义。
- **记忆条目上限**：planner 长期记忆为有界集合（参考 hermes 有界快照）；v1 不设硬上限（调研 D2，冻结快照单页烘焙假设），具体上限值与达上限策略（报错让 LLM 自行 consolidate，调研 §3.2.2）由 plan 决定。
...
- **FR-001**: saolei MCP MUST 将原 `saolei_click`/`saolei_flag`/`saolei_chord_click` 三个落子工具合并为单个 `saolei_operate` 工具，其参数采用与 hermes `memory` 工具一致的双形态（Session 2026-08-08）：MUST 同时支持**普通参数**（单次操作：`type`∈{click/flag/chord} + 坐标 `x`/`y`）与 **`operations` 数组**（批量：`[{type, x, y}, ...]` 有序列表）。两种形态二选一，语义等价（单次等价于长度 1 的 `operations`）。MUST 按声明的顺序依次对每个操作进行既有严格校验并执行，保留操作顺序。同时提供两种形态或均不提供时的拒绝/优先规则由 plan 决定（约束：二者语义等价）。
- **FR-002**: `saolei_operate` MUST 按顺序依次对每个操作进行既有严格校验并执行，并对一次调用（无论单次或批量）只返回一次结果（单一 MCP 文本内容块，反映处理后的最终棋盘与游戏状态）。批量内的失败处理按拒绝原因**细分**（澄清 Session 2026-08-07）：
...
- **FR-004**: 同步给 planner 的游戏历史（gameLog）MUST 以 `saolei_operate` 为单位记录——一次调用（无论单次或批量）记录为一条含其全部操作的历史项；MUST NOT 拆分为多条单步记录。
...
- **FR-007**: 系统 MUST 在 agent 服务上新建 memory mcp server，向 planner 暴露**一个** hermes 风格 `memory` 工具（Session 2026-08-08）；该 mcp server MUST 经 agent 服务将记忆请求转发到 memory 服务，MUST NOT 由 mcp server 直接连接到 memory 服务。
- **FR-008**: planner 持有的 `memory` 工具参数 MUST 与 hermes [`memory` schema](https://github.com/NousResearch/hermes-agent/blob/main/tools/memory_tool.py) 对齐：`action`（`add`/`replace`/`remove`）、`content`（条目正文，add/replace 用）、`old_text`（定位既有条目的短子串，replace/remove 用）、`operations`（批量原子操作数组，每项 `{action, content?, old_text?}`）。MUST NOT 含 `memory_id` 参数（`memory_id` 为 memory 服务内部存储键，由 agent 在 `add` 时内部生成，对 LLM 不可见）；MUST NOT 含 `target` 参数（planner 仅单一记忆存储）。memory 服务的存储 API（`MemoryService` Create/Update/Delete/List，资源 pattern 不变）MUST 保持不变——agent MUST 负责将 hermes 式调用转换为服务的 memory_id 式 RPC：`add` → agent 生成 memory_id + `CreateMemory`；`replace`/`remove` → agent 经 `ListMemories` 取条目、按 `old_text` **子串匹配**定位唯一条目得其 memory_id → `UpdateMemory`/`DeleteMemory`。匹配语义 MUST 同 hermes：**0 命中 → 错误文本（含当前条目列表助 LLM 重选）**；**多个不同条目命中同一子串 → 错误文本（要求更具体子串）**；多条目内容全相同 → 作用于首条；`add` 等价内容已存在 → 成功（去重，非错误）。
- **FR-009**: 仅 `planner` 持有并调用 `memory` 工具；`player` MUST NOT 持有任何记忆工具，MUST NOT 持有任何长期存储。
...
- **FR-011**: 注入 planner 系统提示词的记忆快照中，每条记忆 MUST 以**纯内容**呈现（hermes 风格，无 `memory_id` 前缀；Session 2026-08-08），使 LLM 能据条目内容用 `old_text` 子串调用 `memory(action=replace/remove)`。此与 FR-008（工具无 `memory_id`、用 `old_text` 子串）耦合自洽。
...
- **FR-018**: 本特性 MUST 提供大型测试（large test，经 testplan skill 执行），覆盖关键行为，至少包含：`saolei_operate` 双形态（单次普通参数 / `operations` 数组）执行与单次返回（含无害空操作跳过 / 结构性拒绝停止）；planner 长期记忆经 memory 服务持久化与冻结快照纯内容注入；`memory` 工具 hermes 式参数（`action`/`content`/`old_text`/`operations`，无 `memory_id`）与 agent 转换（含 `old_text` 0/多命中报错）；memory skill 注入 planner（player 不含）；planner 指令发送工具（正常复盘 prompt"必要时才调用" + 消息顺序、team 初始化/压缩后 prompt 引导无历史产出且不触发 player invoke）；player 不再读取策略；memory 服务使用独立数据库；ListMemories 分页。验收标准为所有测试用例全部通过（宪法原则 VI）。
...
- **FR-020（Session 2026-08-08）**: 系统 MUST 为 memory mcp 提供配套 memory skill（`projects/game/agent/src/skill/memory/SKILL.md`，遵循 `specs/020-agent-resources-layout/contracts/skill-md-format.md` 的 frontmatter/body 契约），参考 hermes 记忆工具引导（`research.md` D11）编写，由 planner 系统提示词按既有 skill 机制注入（与 saolei skill 注入 player 对称）。skill 内容 MUST 覆盖：`memory` 工具的用法（`action`/`content`/`old_text`/`operations`，`old_text` 子串定位、0/多命中如何重选）；何时记录记忆（适配复盘域：被监管 agent 的重复错误模式、被验证有效的策略、复盘认知演化）；跳过什么（单次偶发失误、session 特定瞬态、易重推事实）；冻结快照模型（改存储不立即进入系统提示词、压缩边界刷新，FR-010）。skill body 措辞 MUST 领域中立（对齐 hermes 原描述），**不得引用具体模板的角色名（planner/player）、通道名（playerMessages）、调参常量（压缩间隔）或游戏术语**——memory skill 是全局通用资源（资源 pattern 以 template 为段，多模板扩展留位），其内容不随模板而变。memory skill MUST 仅注入 planner（player 不持记忆工具，MUST NOT 注入 player）。

### Key Entities *(include if feature involves data)*

- **Memory（planner 长期记忆条目）**：planner 自身的长期记忆，以会话为作用域（`templates/{template}/sessions/{session}/memories/{memory}`，`{memory}` 段为 memory 服务内部 memory_id），由新建的 memory 服务（grpc-go）承载并持久化。经 memory mcp（部署在 agent 服务上，转发到 memory 服务）暴露的**单一 hermes 风格 `memory` 工具**（参数 `action`/`content`/`old_text`/`operations`，无 `memory_id`；agent 转换为服务 memory_id 式 RPC）增删改。仅 planner 持有；以冻结快照注入 planner 系统提示词（**纯内容**，无 `memory_id` 前缀），压缩边界刷新。取代原共享 Strategy（FR-013）。
...
- **saolei_operate（双形态落子操作）**：合并原 `saolei_click`/`saolei_flag`/`saolei_chord_click` 的单一落子工具，采用 hermes 式双形态参数（普通参数 `type`/`x`/`y` 单次操作 **或** `operations` 数组批量），按序校验执行，单次返回最终棋盘与状态。`saolei_init`/`saolei_remain` 不变。
- **memory 服务（grpc-go）**：新建服务，承载 planner 长期记忆条目的持久化与读写（存储 API：Create/Update/Delete/List，memory_id 式资源，不变）；agent 服务经转发访问，mcp server 不直连。
- **memory mcp server（agent）**：新建于 agent 服务的 mcp server，向 planner 暴露**单一 `memory` 工具**（hermes 式参数），由 agent 将 hermes 式调用转换为 memory 服务的 memory_id 式 RPC 后转发；template/session 经 path 闭包注入。
- **memory skill（planner 专属）**：新建 `projects/game/agent/src/skill/memory/SKILL.md`，参考 hermes 记忆工具引导编写，注入 planner 系统提示词（与 saolei skill 注入 player 对称），指导何时/如何使用 `memory` 工具与冻结快照模型（FR-020）。仅 planner 注入。
...
- **SC-001**: player 落子时，一次 `saolei_operate` 调用（单次普通参数或 `operations` 数组）即可完成一个或多个有序操作并只收到一次结果（相较原先每个落子一次往返，批量场景的调用往返显著减少）。
- **SC-002**: planner 的长期记忆经独立 memory 服务持久化，跨进程重启可读；planner 持有且仅持有**一个** hermes 风格 `memory` 工具（`action`/`content`/`old_text`/`operations`，无 `memory_id`），并能据此逐条增删改自身记忆（agent 转换为服务 memory_id 式 RPC；`old_text` 0/多命中报错）。
- **SC-003**: planner 系统提示词中的长期记忆以**纯内容**冻结快照呈现（无 `memory_id`），跨多次复盘保持冻结，仅在压缩边界刷新；改存储不立即进入快照（冻结语义）。
...
- **SC-006**: memory mcp server 部署在 agent 服务上并由 agent 转发请求到 memory 服务，mcp server 不直连 memory 服务（连接拓扑可验证）；memory 服务存储 API 不变。
- **SC-007**: 同步给 planner 的游戏历史以 `saolei_operate`（含全部操作）为单位记录，注入 planner 的工具描述与历史中的操作类型一一对应。
- **SC-009（Session 2026-08-08）**: memory skill 注入 planner 系统提示词（player 不注入），引导 `memory` 工具用法、冻结快照模型与复盘域记忆取舍；可经 planner 系统提示词审查验证。
- **SC-008**: 大型测试（经 testplan skill 完整部署→测试→清理执行）全部用例通过，覆盖 SC-001/SC-002/SC-003/SC-004/SC-009 所述行为。
...
- **`memory_add` 的 memory_id 由调用方提供，冲突时拒绝** ~~（已 supersede，Session 2026-08-08）~~：原 2026-08-07 决策要求 `memory_add` 含 `memory_id`、冲突拒绝。**Session 2026-08-08 推翻**：memory 工具改为 hermes 式单一 `memory` 工具（`action`/`content`/`old_text`/`operations`，无 `memory_id` 参数）；`memory_id` 退化为 memory 服务内部存储键，由 agent 在 `add` 时内部生成（非 LLM 提供），对 LLM 不可见；`replace`/`remove` 经 `old_text` 子串定位（0/多命中报错，详见 FR-008 与 Edge Cases）。memory 服务存储 API（memory_id 式资源）不变。详见 Clarifications Session 2026-08-08 与 `research.md` D11。
- **memory 工具不设 `target` 参数（Session 2026-08-08）**：hermes `memory` 工具有 `target`（memory/user 双存储）；dominion planner 仅有**单一**记忆存储（自身长期复盘记忆），无第二存储，故 `target` 无意义、不设。此为对"与 hermes 相同参数"的有意裁剪（单存储场景下的合理简化）。
- **saolei_operate 双形态参数的等价性（Session 2026-08-08）**：`saolei_operate` 普通参数（单次 `type`/`x`/`y`）与 `operations` 数组语义等价（单次 = 长度 1 的 operations）。同时提供两者或均不提供的拒绝/优先规则留 plan 决定（约束：二者语义等价）。空 `operations` 列表行为留 plan 决定（约束：不产生落子副作用）。
...
- **记忆条目上限与 session 删除清理**：planner 长期记忆为有界集合；v1 不设硬上限（调研 D2，冻结快照单页烘焙假设），上限值与达上限策略（参考 hermes 报错让 LLM 自行 consolidate）留待 plan；session 删除时 memory 条目是否级联清理留待 plan（与 031 对 strategy"暂不清理"对齐，后续优化）。
...
- **参考资料**：前期调研见 `survey/planner-memory-and-agent-communication.md`（hermes 冻结快照/记忆工具/压缩刷新、openclaw HumanMessage 投递、LangGraph 通道）；hermes 记忆工具参数与引导调研见 `research.md` D11（来源 [hermes `tools/memory_tool.py`](https://github.com/NousResearch/hermes-agent/blob/main/tools/memory_tool.py) `MEMORY_SCHEMA`）。相关现有契约：`specs/031-team-template-mode/contracts/strategy-store-contract.md`（待废弃）、`specs/031-team-template-mode/contracts/team-graph-contract.md`（player/planner 节点、策略/记忆流）、`specs/031-team-template-mode/contracts/saolei-sink-contract.md`（游戏事件 sink）、`specs/020-agent-resources-layout/contracts/skill-md-format.md`（SKILL.md 格式契约，FR-020 引用）。相关现有代码：`projects/game/agent/src/mcp/saolei/saolei-mcp.ts`（落子工具、`SaoleiEventSink`）、`projects/game/agent/src/team/{player,planner}.ts`、`projects/game/agent/src/strategy-store.ts`（待废弃）、`projects/game/agent/src/mcp-host.ts`（mcp server path 闭包注入既有做法）、`projects/game/agent/src/skill-loader.ts`（skill 注入机制，FR-020 引用）、`projects/game/agent/src/server.ts`（服务装配）。
