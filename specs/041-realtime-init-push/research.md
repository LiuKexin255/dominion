...
- The typing indicator is already driven only by real user turns: the connect status probe uses `deriveStatusSignal(team?.isRunning() …)` (`handler.ts:384-387`), and `isRunning()` **deliberately excludes** `initInFlight` (`session-team.ts:392-411`); `isBusy()` gates destructive ops (`session-team.ts:425-427`). The `isRunning`/`isBusy` split is already implemented and committed on the 040 branch HEAD `868c49b` (spec Assumption line 106).
