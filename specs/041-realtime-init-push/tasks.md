...
- Desktop Go tests: `style/golang.md` — table-driven, `given/when/then`, no assertions inside cases, `TestFuncName`/`Test_funcName` naming; the existing `app_test.go` `recvLoop` test pattern (`app_test.go:556-601`) is the model.
...
**Purpose**: establish the feature branch from a working-tree base that already contains the prerequisite `isRunning`/`isBusy` split (spec Assumption line 106) and the 039/040 changes this feature builds on.
...
- [ ] T001 Create/switch to feature branch `041-realtime-init-push` from the current working tree (which contains the uncommitted `isRunning`/`isBusy` split in `projects/game/agent/src/session-team.ts` + `handler.ts` and the 039/040 baseline). Verify the base compiles: `bazel build //projects/game/agent:agent_lib //projects/game/desktop:desktop_lib` and the existing tests pass: `bazel test //projects/game/agent:agent_test //projects/game/desktop:desktop_test`. If the split is not present, stop and surface the gap (it is a hard prerequisite for US2).
...
4. **STOP and VALIDATE**: run US1's Independent Test (agent emits the 3 frames with correct `agent`/`role`/`frameId`); run `bazel test //projects/game/agent:agent_test //projects/game/desktop:desktop_test`. The init instruction is now visible on first entry — demo-ready.
