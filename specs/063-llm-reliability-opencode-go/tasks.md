...
- [X] T021 [P] 在 `projects/game/testplan/agent_v2_preset_test.go`（preset/目录模块）更新断言：ListModels 联合目录（两 provider 复合标识、同名 `glm-5.3` 消歧）、UpdateTeam 复合标识/裸 id/空值三分支、GetTeam 成员 `model` 回填复合标识（FR-018，`specs/063-llm-reliability-opencode-go/contracts/model-selection.md` §3）；`bazel build //projects/game/testplan/...` 编译通过
