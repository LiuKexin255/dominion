...
- [ ] T009 [US2] 新增 `common/js/dsh-plugins/preset-authoring/src/derive.ts`：`deriveComposition(store 记录, 模板组合)` → 组合行列表（persona 行 `config.text` ← 记录 persona；空 persona 保留模板原文=默认 base）→ `mkdtemp(os.tmpdir())` 写 `agent.cordis.yml` → 返回合成 `AgentPreset {id, trust: 'user', path}`；幂等纯函数；新增 `derive.test.ts`（persona 替换/空值回退/幂等重建/临时文件内容）
- [ ] T010 [US2] `common/js/dsh-plugins/preset-authoring/src/index.ts`：`compose(presetId)` 改为 `store.get` → `deriveComposition` → setup 经根导出的 `mountPreset(agentCtx, preset)` 挂载（import 自 `@deepseek-ai/dsh-agent-presets`；fail-fast 语义与 `agentPreset` meta 不变）；`index.test.ts` 更新（compose 不再 resolve/copy 用户 preset；NOT_FOUND/幂等用例）
