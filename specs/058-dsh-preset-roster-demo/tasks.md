...
- [X] T004 [P] demo-echo 插件：`experimental/dsh/demo/agent-plugins/demo-echo/src/index.ts`（`demo_echo` 工具 + guidance section 同 `apply()` 注册，inject `["tools","systemPrompt"]`）+ `src/index.test.ts`（mock registry 断言两者成对注册）
- [X] T005 [P] preset-authoring Store：`common/js/dsh-plugins/preset-authoring/src/store.ts`（`PresetStore` 接口 + 内存实现 + 稳定错误码 ALREADY_EXISTS/NOT_FOUND）+ `src/store.test.ts`
- [X] T006 preset-authoring 物化：`common/js/dsh-plugins/preset-authoring/src/materialize.ts`（roster `copy(template, id, displayName)` → js-yaml round-trip patch persona 行 `config.text`（原子写）→ 失败回滚目录；update 的 persona/preset.yml 分流）+ `src/materialize.test.ts`（mock roster/fs seam：patch 正确性、回滚无残留、模板约定违约报错）
- [X] T007 preset-authoring 服务装配：`common/js/dsh-plugins/preset-authoring/src/index.ts`（四导出 + `ctx.presetAuthoring` 注册：`compose/create/get/list/update/remove`，contracts §2 接口）+ `src/index.test.ts`（compose 返回形状、错误码映射；V3-3：fs seam 写坏副本组合文件 → resolve/list 报 broken、compose fail-fast 不产生半组合会话）
- [X] T008 [P] 模板 preset 数据：`experimental/dsh/demo/agent/presets-templates/demo-standard/{agent.cordis.yml,preset.yml}` 与 `demo-tools/{agent.cordis.yml,preset.yml}`（contracts/composition-manifest.md §3；遵守模板约定：无注释/无 !!js/唯一 persona 行）
- [X] T009 [P] proto 扩展：`experimental/dsh/demo/chat.proto` 增 `Chat.CreateConversation` 与 `PresetService` 五 RPC（含 `google.api.http` 注解，contracts/chat-api.md）→ 重生成 `ts_proto_library` 类型与 gateway 产物（`experimental/dsh/demo/gateway/`）
- [X] T010 [P] 组合改造：`experimental/dsh/demo/agent/cordis.yml` 按 contracts/composition-manifest.md §1 重写（spine → 直组 11 行；roster roots 经 `PRESET_TEMPLATES_ROOT`/`PRESET_WRITABLE_ROOT` env）
- [X] T011 BUILD 与部署面：`experimental/dsh/demo/agent/BUILD.bazel`（npm_deps 增删 + `data_files` 增 `presets-templates` + vitest data 镜像同步）；`experimental/dsh/demo/testplan/deploy.yaml` 注入两 roster env（writable root 为容器临时可写层固定路径，deploy schema 无用户卷通道，contracts/composition-manifest.md §2）；`experimental/dsh/demo/testplan/closure_audit_test.go` expected 集随 package.json 重算
