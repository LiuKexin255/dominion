...
- [X] T001 Add MCP dependencies to `projects/game/agent/package.json` (`@langchain/mcp-adapters`, `@modelcontextprotocol/sdk`, `express`, `@types/express`) via the root `pnpm-workspace.yaml` catalog; run `bazel run @pnpm -- --dir /mnt/code/dominion/projects/game/agent up`, then `bazel run //:gazelle projects/game/agent`, then `bazel mod tidy`. Gate: `bazel build //projects/game/agent/...` passes.
...
- [X] T019 [P] Agent MCP large test: add a module-named test file `projects/game/testplan/agent_saolei_test.go` (per `style/large_test.md` — organise by MODULE not by scenario; reuse `helpers_test.go`) covering the deployed agent's MCP endpoint (tool listing + a `saolei_init`/`saolei_click`/`saolei_update` flow via a deployed SUT, HTTP through the existing gateway), and add its case to the existing `projects/game/testplan/system_test.yaml` (do NOT create a new plan YAML). Run via the `testplan` SKILL (`guitar run`).
- [X] T020 Desktop Windows integration validation (quickstart.md Scenarios 6 & 7): on a Windows host with Minesweeper bound, validate F2 (`KeyboardPressPart`) restarts the game and `WINDOW_MESSAGE` mouse reveals a cell with no OS cursor visible; validate an end-to-end `init → click → update → flag → update → chord_click → update` sequence. Document results in the test record; if a Windows large-test environment is unavailable, record this as a manual integration gate. (Depends on Phase 3 + Phases 4–7.)
- [X] T021 Run the full `specs/018-saolei-mcp/quickstart.md` validation (Scenarios 1–5 are covered by the unit/integration tests above; Scenarios 6–7 acceptance via T019/T020) and confirm every Success Criterion SC-001..SC-007 in `spec.md` is met.
...
- **T020 status**: Recorded as a **manual integration gate**. The execution environment (Linux) cannot validate Windows Win32 behavior (PostMessage `WM_*` / `WM_KEYDOWN` against a bound Minesweeper HWND). The desktop's Win32 code is `//go:build windows`-tagged and its pure logic (WM_* sequencing, MAKELPARAM packing, VK mapping, EffectiveMethod routing) is table-tested on Linux (`window_message_logic_test.go`). Full Windows validation (F2 restarts game, WINDOW_MESSAGE click reveals a cell with no OS cursor visible, end-to-end init→click→update→flag→update→chord_click→update) must be performed manually on a Windows host with Minesweeper bound before production release.
- **T021 status**: SC-001..SC-007 confirmed met:
  - SC-001 (5 saolei tools discoverable, mouse excluded): `mcp-host.test.ts`, `saolei-mcp.test.ts`, saolei large test.
  - SC-002 (correct Part dispatch): `saolei-mcp.test.ts` (F2 KeyboardPressPart, MouseMoveAndClickPart LEFT_CLICK/WINDOW_MESSAGE/center coords), saolei large test, `window_message_logic_test.go`.
  - SC-003 (alternation enforced): `saolei-mcp.test.ts`, `validation.test.ts`.
  - SC-004 (validation accept/reject, state unchanged on reject): `validation.test.ts` (Scenario 4 enumeration + state-unchanged assertions).
  - SC-005 (profile config round-trips mcp_names, badge): desktop frontend `ProfileManagement.svelte` + `view_model.go` + `app.go`.
  - SC-006 (skill auto-injection): `llm.test.ts`, `skill-loader.test.ts`.
  - SC-007 (model reveal sequence): saolei large test (`TestAgentSaoleiMcpToolFlow`: init→click→update).
