...
- **Documentation First (§V)**: per the v4.0.0 constitution, §V is enforced **per phase** — the exported `tasks.md` MUST declare a Required Reading block INSIDE EACH `## Phase N` section, not a single global declaration. This plan seeds that per-phase reading (the feature's own design docs under `specs/018-saolei-mcp/` are loaded by the implementation workflow and are NOT declared). Each phase declaration inherits the subset of the lists below relevant to that phase and MAY add phase-specific items; a document relevant to several phases is repeated in each (no cross-phase inheritance). Every entry MUST resolve to a concrete file path or link, every external entry MUST carry its own inline link, each phase's declaration MUST include all three categories (stating "None" explicitly when empty) and MUST cover context broader than that phase's edit sites. The task planner MUST read the in-repo docs each phase's change touches and follow their in-repo references before authoring that phase's declaration. The seed categories:
