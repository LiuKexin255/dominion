...
- **definition**: declared via the `vitest_test` macro — `vitest_test(name = "lib_test", data = [...])`. The caller's `data` MUST be the package's **raw `.ts` source** (`glob(["src/**/*.ts"])`, production + tests) PLUS the `:node_modules/*` deps that mirror the package's own `:lib` `deps` (Fix B — dropping `:lib` from `data` also removes the transitive `node_modules` it carried into runfiles; external npm packages have no competing `.ts` source, so adding them does NOT re-create the dual instance). `:lib` itself MUST NOT be in `data` (its compiled `.js` alongside the `.ts` source re-creates the dual-module-instance root cause — see [research.md](research.md) §6). The macro injects the `entry_point` (the per-package `genrule`-generated `…_run_vitest.mjs`), `args` (`["run","src/"]` default), and `:node_modules/vitest` into `data`. Each package runs `npm_link_all_packages(name = "node_modules")` so `vitest` is resolvable from the generated copy's location. **Trade-off (acknowledged):** the test executes vitest's on-the-fly transpilation of the source, NOT the swc-compiled `:lib` artifact; compile/type correctness is retained — `:lib` (`ts_project`) is still built as a dependency of `server_pkg`, so `bazel build //...:server_pkg` still type-checks and compiles.
