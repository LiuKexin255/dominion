...
**Organization**: 按 user story 组织（US1 游戏闭环 P1 MVP / US2 preset 与物化 P1 / US3 desktop 退化 P2 / US4 web 侧栏 P2）；跨故事共享的阻塞前置归 Foundational；v1 处置与全量验收为横切收尾 phase。
...
- [ ] T023 [US1] US1 验收执行：`guitar validate projects/game/testplan/system_test.yaml` + `guitar run projects/game/testplan/system_test.yaml --suite agent-v2-game` 与 `--suite desktop-flow`——部署→测试→清理闭环、全部用例通过（失败用 signoz skill 查 tracing/log 排查修复后重跑至 green；constitution 原则 VI）
...
- **US2 (Phase 4)**: 依赖 Phase 3 的 T014/T015/T017/T018（AgentService 面与 web 适配就绪）；T024 → T025 → T026 → T027 → T028（T025/T026 均集成于 `App.tsx`，串行执行避免同文件冲突）
