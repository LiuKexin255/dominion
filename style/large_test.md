...
* 一个 `*_test.go` 文件聚焦一个被测模块；模块按照设计层级、功能内聚程度进行划分，小的模块例如某个资源的 CURD，大的模块例如整个系统，模块的拆分应当覆盖从小到大不同的尺度。每个模块的测试用例应当依据当前模块的**职能**和**目标**进行编写。
* 不要为了"凑齐一组验收场景"把多个模块的测试堆在同一个文件。
* 共享的构造/断言代码放入共享 helper 文件（如 `helpers_test.go`），避免在多个文件中重复定义。

**按 suite 编排**：

* 一个 `suite` 对应一组部署拓扑与几组聚焦用例。可以将使用相同部署拓扑且互不干扰的测试用例放在一个case。
* 在“较多的部署拓扑但部署的服务少”和“较少的部署拓扑但每部署的服务多”之前取得平衡。前者部署次数是较多但每次部署精简，干扰较少；后者部署次数少，部署一次可以执行多个模块的测试，但多个模块的测试可能产生干扰或 flaky 用例。
