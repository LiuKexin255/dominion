...
2. **依赖治理——统一 catalog**：宪法要求 TS 依赖版本统一在 `pnpm-workspace.yaml` catalog；dsh 依赖统一入 catalog（含 `third_party/dsh/core` 底座闭包）：rc 线精确版本（`"x.y.z-rc.n"` 无前缀）、cordis/schemastery 保持 range、cordis-plugin 家族与 node-addon-require-builtin 精确版本——dsh 闭包整体即一个版本单元。
...
│                                           + node-addon-require-builtin；版本经 catalog 统一管理，§4-2）
...
1. **baseline workspace 包**（§5.1/§5.6，唯一必须项）：`third_party/dsh/core`（框架核心 ≈10 包精确 pin，版本经 catalog 统一管理）+ 按需的场景 baseline 包（如 `coding`，`runtime_deps=[core]` + 增量枚举；枚举可从各自 package.json 脚本生成）。
